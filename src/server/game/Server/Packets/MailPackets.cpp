...
WorldPackets::Mail::MailListEntry::MailListEntry(::Mail const* mail, ::Player* player)
{
    ...
    time_t correct_time = mail->expire_time;

    // alistar: undelivered auctions will use deliver time to show in auction UI
    if (mail->messageType == MAIL_AUCTION)
        if (mail->deliver_time > GameTime::GetGameTime())
            correct_time = mail->deliver_time;

    DaysLeft = float(correct_time - GameTime::GetGameTime()) / float(DAY);
