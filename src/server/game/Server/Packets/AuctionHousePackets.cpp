...
void AuctionBrowseQuery::Read()
{
    ...
    if (uint32(Filters) != uint32(-1))
        Filters |= AuctionHouseFilterMask(1 << (uint32(Filters) + 4));
    ...
    if (OnlyUsable = _worldPacket.ReadBit())
        Filters |= AuctionHouseFilterMask::UsableOnly;
    else
        Filters &= ~AuctionHouseFilterMask::UsableOnly;

    if (ExactMatch = _worldPacket.ReadBit())
...
void AuctionListBiddedItems::Read()
{
    _worldPacket >> Auctioneer;
    _worldPacket >> Offset;
    AuctionItemIDsCount = _worldPacket.ReadBits(7);
}
...
void AuctionListItems::Read()
{
    ...
    _worldPacket >> KnownPetsCount;

    _worldPacket >> MaxPetLevel;

    // TODO knownPets

    Unknown343 = _worldPacket.ReadBit();

    uint32 nameLength = _worldPacket.ReadBits(8);
    Name = _worldPacket.ReadString(nameLength);

    ClassFilterCount = _worldPacket.ReadBits(3);
...
void AuctionSellItem::Read()
{
    ...
    // alistar: these 6 bits are the Items size but in classic we only use 1
    _worldPacket.ReadBits(6);
...
{
    _worldPacket << Guid;
    _worldPacket << uint32(AuctionHouseID);
...
void AuctionListPendingSales::Read()
{
    // alistar: intended to be empty
}
