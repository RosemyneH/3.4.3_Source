...
void MapManager::PreloadContinents()
{
    const uint32 total_load_time = getMSTime();

    const uint32 expansion_id = sConfigMgr->GetIntDefault("Expansion", CURRENT_EXPANSION);
    std::vector<std::future<void>> futures;
    std::mutex map_mutex;

    enum { MAP_EASTERN_KINGDOMS = 0, MAP_KALIMDOR = 1, MAP_OUTLAND = 530, MAP_NORTHREND = 571 };
    for (const uint16 map_id : { MAP_EASTERN_KINGDOMS, MAP_KALIMDOR, MAP_OUTLAND, MAP_NORTHREND })
    {
        futures.push_back(std::async(std::launch::async, [this, map_id, expansion_id, &map_mutex]()
        {
            if (map_id == MAP_OUTLAND && expansion_id < EXPANSION_THE_BURNING_CRUSADE)
                return;

            if (map_id == MAP_NORTHREND && expansion_id < EXPANSION_WRATH_OF_THE_LICH_KING)
                return;

            Map* map = nullptr;
            {
                std::lock_guard<std::mutex> lock(map_mutex);
                map = FindMap_i(map_id, 0);
            }

            if (map == nullptr)
            {
                const uint32 map_load_time = getMSTime();
                TC_LOG_INFO("server.loading", ">> Loading Map: {}", map_id);

                if ((map = CreateWorldMap(map_id, 0)))
                {
                    std::lock_guard<std::mutex> lock(map_mutex);
                    i_maps[{ map->GetId(), 0 }] = map;
                    TC_LOG_INFO("server.loading", ">> Loaded Map: {} in {} ms", map_id, GetMSTimeDiffToNow(map_load_time));
                }
                else
                {
                    TC_LOG_ERROR("", "Failed to create map for map_id: {}", map_id);
                }
            }
        }));
    }
    ...
    TC_LOG_INFO("server.loading", ">> Preloaded continents in {} ms", GetMSTimeDiffToNow(total_load_time));
}
