...
InstanceResetTimePoint InstanceLockMgr::GetNextResetTime(MapDb2Entries const& entries)
{
    ...
    switch (entries.MapDifficulty->ResetInterval)
    {
        ...
        case MAP_DIFFICULTY_RESET_EVERY_3_DAYS:
        {
            int32 daysSinceEpoch = (std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()) / 86400);
            int32 daysToAdd = (3 - (daysSinceEpoch % 3));
        ...
        case MAP_DIFFICULTY_RESET_EVERY_5_DAYS:
        {
            int32 daysSinceEpoch = (std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()) / 86400);
            int32 daysToAdd = (5 - (daysSinceEpoch % 5));
