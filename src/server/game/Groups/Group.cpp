...
itemRandomPropId(li.randomProperties), itemRandomSuffix(li.randomSuffix), itemCount(li.count),
...
m_dungeonDifficulty(DIFFICULTY_NORMAL), m_raidDifficulty(DIFFICULTY_RAID_10_N), m_legacyRaidDifficulty(DIFFICULTY_10_N),
...
m_readyCheckStarted(false), m_readyCheckTimer(Milliseconds::zero()), m_activeMarkers(0), _team(0)
{
...
bool Group::Create(Player* leader)
{
    ...
    m_raidDifficulty = DIFFICULTY_RAID_10_N;
...
void Group::SendLootStartRollToPlayer(uint32 countDown, uint32 mapId, Player* p, bool canNeed, Roll const& r) const
{
    ...
    startLootRoll.RollTime = Milliseconds(countDown);
            ...
            if (!maxguid.IsEmpty())
            {
                ...
                if (player && player->GetSession())
                {
                    ...
                    if (msg == EQUIP_ERR_OK)
                    {
                        ...
                        player->StoreNewItem(dest, roll->itemid, true, item->randomProperties, item->GetAllowedLooters(), item->context/*, item->BonusListIDs*/);
                    }
                    else
                    {
            ...
            if (!maxguid.IsEmpty())
            {
                ...
                if (player && player->GetSession())
                {
                    ...
                    else if (rollvote == DISENCHANT)
                    {
                        ...
                        InventoryResult msg = player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, roll->itemid, item->count);
                        if (msg == EQUIP_ERR_OK)
                            player->AutoStoreLoot(pProto->DisenchantID, LootTemplates_Disenchant, ItemContext::NONE, true);
                        else // If the player's inventory is full, send the disenchant result in a mail.
                        {
                            //Loot loot(allowedMap, roll->getLoot()->lootOwnerGUID, LOOT_DISENCHANTING);
                            Loot loot;
...
ItemTemplate const* Roll::GetItemDisenchantLoot() const
{
    if (LootItem const* lootItemInSlot = (*this)->GetItemInSlot(itemSlot))
    {
...
void Group::SetDungeonDifficultyID(Difficulty difficulty)
{
    ...
    // alistar: send an update after setting dungeon difficulty updates client side.
    SendUpdate();
}
...
void Group::SetRaidDifficultyID(Difficulty difficulty)
{
    ...
    // alistar: send an update after setting raid difficulty updates client side.
    SendUpdate();
}
