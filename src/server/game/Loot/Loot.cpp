...
Loot::Loot() 
    : gold(0), unlootedCount(0), roundRobinPlayer(), loot_type(LOOT_NONE), maxDuplicates(1),
    _guid(ObjectGuid::Empty), _itemContext(ItemContext::NONE), containerID(0)
{
...
void Loot::NotifyItemRemoved(uint8 lootIndex, Map const* map)
{
    ...
    for (auto itr = PlayersLooting.begin(); itr != PlayersLooting.end();)
    {
        ...
        {
            player->SendNotifyLootItemRemoved(GetGUID(), lootOwnerGUID, lootIndex);
            ++itr;
        }
        else
    ...
    for (auto itr = PlayersLooting.begin(); itr != PlayersLooting.end();)
    {
        if (Player* player = ObjectAccessor::GetPlayer(map, *itr))
        {
            ...
            if (pq != PlayerQuestItems.end() && pq->second)
            {
                ...
                if (j < pql.size())
                    player->SendNotifyLootItemRemoved(GetGUID(), lootOwnerGUID, items.size() + j);
            }
...
bool Loot::FillLoot(uint32 lootId, LootStore const& store, Player* lootOwner, bool personal, bool noEmptyError, uint16 lootMode /*= LOOT_MODE_DEFAULT*/, ItemContext context /*= ItemContext::NONE*/)
{
    ...
    if (Map* map = lootOwner->GetMap())
        _guid = ObjectGuid::Create<HighGuid::LootObject>(map->GetId(), 0, map->GenerateLowGuid<HighGuid::LootObject>());
...
void Loot::AddItem(LootStoreItem const& item)
{
    ...
    for (uint32 i = 0; i < stacks && lootItems.size() < limit; ++i)
    {
        ...
        if (!item.needs_quest && item.conditions.IsEmpty() && !proto->HasFlag(ITEM_FLAG_MULTI_DROP))
            ++unlootedCount;
    }
}
...
LootItem* Loot::LootItemInSlot(uint32 lootSlot, Player* player, NotNormalLootItem* *qitem, NotNormalLootItem* *ffaitem, NotNormalLootItem* *conditem)
{
    ...
    else
    {
        ...
        else if (!item->conditions.IsEmpty())
        {
...
uint32 Loot::GetMaxSlotInLootFor(const Player* player) const
{
    NotNormalLootItemMap::const_iterator itr = PlayerQuestItems.find(player->GetGUID());
...
void Loot::BuildLootResponse(WorldPackets::Loot::LootResponse& packet, Player* viewer, PermissionTypes permission) const
{
    ...
    switch (permission)
    {
        ...
        case RESTRICTED_PERMISSION:
        {
            ...
            for (uint8 i = 0; i < items.size(); ++i)
            {
                if (!items[i].is_looted && !items[i].freeforall && items[i].conditions.IsEmpty() && items[i].AllowedForPlayer(viewer))
                {
                    uint8 slot_type;
        ...
        case ROUND_ROBIN_PERMISSION:
        {
            for (uint8 i = 0; i < items.size(); ++i)
            {
                if (!items[i].is_looted && !items[i].freeforall && items[i].conditions.IsEmpty() && items[i].AllowedForPlayer(viewer, roundRobinPlayer))
                {
                    ...
                    WorldPackets::Loot::LootItemData lootItem;
                    lootItem.LootListID = i + 1;
                    lootItem.UIType = permission == LOOT_SLOT_TYPE_ALLOW_LOOT;
                    lootItem.Quantity = items[i].count;
                    lootItem.Loot.Initialize(items[i]);
                    packet.Items.push_back(lootItem);
                }
        ...
        if (!item->is_looted && item->freeforall && item->AllowedForPlayer(player, lootOwnerGUID))
            if (ItemTemplate const* proto = sObjectMgr->GetItemTemplate(item->itemid))
                if (proto->IsCurrencyToken())
                    player->StoreLootItem(ObjectGuid::Empty, i, this);
    }
}
