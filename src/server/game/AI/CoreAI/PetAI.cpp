...
void PetAI::UpdateAI(uint32 diff)
{
    ...
    {
        if (owner && owner->IsPlayer() && me->GetCharmInfo()->GetForcedSpell() && !me->GetCharmInfo()->GetForcedTarget().IsEmpty())
        {
        ...
        for (uint8 i = 0; i < me->GetPetAutoSpellSize(); ++i)
        {
            ...
            if (spellInfo->IsPositive())
            {
                ...
                if (spellInfo->CanBeUsedInCombat(me) && spellInfo->GetDuration() != -1 && !me->IsInCombat() && !me->GetCharmInfo()->IsCommandAttack())
                    continue;
...
void PetAI::HandleReturnMovement()
{
    ...
    me->GetCharmInfo()->SetForcedTargetGUID(ObjectGuid::Empty);
...
void PetAI::DoAttack(Unit* target, bool chase)
{
    ...
    if (me->Attack(target, true))
    {
        ...
        if (chase)
        {
            ...
            {
                // alistar: pets shouldn't attack players from behind
                if (target)
                {
                    if (target->ToPlayer())
                    {
...
void PetAI::ReceiveEmote(Player* player, uint32 emote)
{
    ...
    switch (emote)
    {
        ...
        case TEXT_EMOTE_KISS:
            if (me->IsPet() && me->ToPet()->IsPetSayaad())
            {
                static constexpr uint32 SPELL_ENCHANTED = 60580;
                if (!player->GetSpellHistory()->HasCooldown(SPELL_ENCHANTED))
                {
                    me->SetFacingToObject(player);
                    player->CastSpell(me, SPELL_ENCHANTED, false);
                }
            }
            break;
    }
...
bool PetAI::CanMeleeAttack() const
{
    return me->GetEntry() != NPC_IMP && 
           me->GetEntry() != NPC_WATER_ELEMENTAL && 
           me->GetEntry() != NPC_WATER_ELEMENTAL_P;
}
...
void PetAI::SpellHit(WorldObject* caster, const SpellInfo* spellInfo)
{
