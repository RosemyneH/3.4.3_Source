...
void SmartScript::OnReset()
{
    ...
    if (me)
    {
        me->SetIsFleeingForAssist(false);
        me->SetHasFledForAssist(false);
    }
}
...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, SpellInfo const* spell, GameObject* gob, std::string const& varString)
{
    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_CAST:
        {
            ...
            // alistar: don't allow casting when about to flee for assist
            if (me && me->IsFleeingForAssist())
                break;
...
void SmartScript::UpdateTimer(SmartScriptHolder& e, uint32 const diff)
{
    ...
    if (e.timer < diff)
    {
        ...
        // Delay flee for assist event if stunned or rooted or casting a spell
        else if (e.GetActionType() == SMART_ACTION_FLEE_FOR_ASSIST)
        {
            if (me)
            {
                if (me->HasUnitState(UNIT_STATE_CASTING) && uint32(me->GetHealthPct()) <= 15 && !me->HasFledForAssist())
                {
                    me->SetIsFleeingForAssist(true);
                    e.timer = 1;
                    return;
                }

                if (me->HasUnitState(UNIT_STATE_ROOT | UNIT_STATE_LOST_CONTROL))
                {
                    e.timer = 1;
                    return;
                }
