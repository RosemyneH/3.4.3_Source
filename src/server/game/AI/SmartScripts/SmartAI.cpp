...
void SmartAI::AttackStart(Unit* who)
{
    ...
    if (who && me->Attack(who, true))
    {
        ...
        // alistar: send pet to attack
        for (Unit* controlled : me->m_Controlled)
            if (Creature* cControlled = controlled->ToCreature())
                if (CreatureAI* controlledAI = cControlled->AI())
                    if (!cControlled->IsInCombatWith(who))
                        controlledAI->OwnerAttackedBy(who);
...
void SmartAI::SetFollow(Unit* target, float dist, float angle, uint32 credit, uint32 end, uint32 creditType)
{
    ...
    if (dist > 0.f && angle > 0.f)
        me->GetMotionMaster()->MoveFollow(target, dist, angle, true);
    else
        me->FollowTarget(target);
}
