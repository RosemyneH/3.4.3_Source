...
void SmartAIMgr::LoadSmartAIFromDB()
{
    ...
    do
    {
        ...
        // alistar: store active creatures to be activated on summon here..
        if (temp.action.type == SMART_ACTION_SET_ACTIVE)
        {
            bool isGUID = temp.entryOrGuid <= 0;
            sObjectMgr->AddActiveCreature(isGUID ? -temp.entryOrGuid : temp.entryOrGuid, isGUID);
        }
    }
    while (result->NextRow());
