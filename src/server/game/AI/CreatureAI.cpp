...
void CreatureAI::MoveInLineOfSight(Unit* who)
{
    ...
    // Northshire Abbey - wolves hack fix. They can call assistance only outside the starting zone.
...
bool CreatureAI::_EnterEvadeMode(EvadeReason /*why*/)
{
    ...
    me->CombatStop(true);
    //me->LoadCreaturesAddon(); alistar: I think we need to apply creatureaddon only when they reach their home pos
...
void CreatureAI::CheckMeleeRepositionRequirements()
{
    ...
    {
        float MaxRange = (me->GetCombatReach() / 2) + (victim->GetCombatReach() / 2);
        if (me->IsInDist(victim, MaxRange))
