...
const std::unordered_map<uint32, uint32> g_ItemToCurrencyStore =
{
    ...
    { 43307, 1900 }, // Arena Points	
    { 43308, 1901 }, // Honor Points

    // alistar: TODO these two don't exist yet
    //{ 29434, 2589 }, // Sidereal Essence
    //{ 29434, 2711 }, // Defiler's Scourgestone
};

constexpr uint32 CHARTER_DISPLAY_ID = 16161;
constexpr uint32 GUILD_CHARTER_ITEM_ID = 5863;

float const GROUND_HEIGHT_TOLERANCE = 0.05f; // Extra tolerance to z position to check if it is in air or on ground.
constexpr float Z_OFFSET_FIND_HEIGHT = 0.5f;

// alistar:
constexpr float DEFAULT_LEEWAY_RANGE = 2.66666667f;
...
{   // (C) used in caster aura state     (T) used in target aura state
    ...
    AURA_STATE_JUDGEMENT                    = 5,            // C  t|
    AURA_STATE_WOUNDED_25_PERCENT           = 6,            //   T |
    AURA_STATE_DEFENSIVE_2                  = 7,            // Cc  |
...
#define PER_CASTER_AURA_STATE_MASK (\
    (1<<(AURA_STATE_CONFLAGRATE-1))|(1<<(AURA_STATE_ROGUE_POISONED-1)))
...
// CreatureFamily.dbc (3.4.3.54261)
enum CreatureFamily
{
...
enum HolidayIds
{
    ...
    HOLIDAY_LOVE_IS_IN_THE_AIR_NEW                      = 423,
...
enum SkillType
{
    ...
    SKILL_BEAST_TRAINING          = 261, // alistar: new
...
constexpr SkillType SkillByLockType(LockType locktype)
{
    switch (locktype)
    {
        case LOCKTYPE_LOCKPICKING: return SKILL_LOCKPICKING;
...
// Indexes of BattlemasterList.db2 (3.4.3.54261)
