    ...
    for (auto itr = _generators.begin(); itr != _generators.end(); ++itr)
    {
        ...
        switch (type)
        {
            ...
            case FOLLOW_MOTION_TYPE:
                if (AbstractPursuer* followInformation = dynamic_cast<AbstractPursuer*>(movement))
                {
...
void MotionMaster::MoveTargetedHome()
{
    ...
    {
        // alistar: if our owner is dead return to home.
        if (!target->IsAlive())
        {
            Add(new HomeMovementGenerator<Creature>());
        }
...
void MotionMaster::MoveChase(Unit* target, Optional<float> dist /*= std::nullopt*/, Optional<ChaseAngle> angle /*= std::nullopt*/)
{
    ...
    Add(new ChaseMovementGenerator(target, dist, angle), MOTION_SLOT_ACTIVE);
}
    ...
    Movement::SpellEffectExtraData const* spellEffectExtraData /*= nullptr*/)
{
    ...
    MoveCharge(dest.x, dest.y, dest.z, speed, EVENT_CHARGE_PREPATH, false, target);
