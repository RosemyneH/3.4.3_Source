...
{
    return target->HasUnitMovementFlag(MOVEMENTFLAG_FORWARD | MOVEMENTFLAG_BACKWARD | MOVEMENTFLAG_STRAFE_LEFT | MOVEMENTFLAG_STRAFE_RIGHT)
...
bool FollowMovementGenerator::Update(Unit* owner, uint32 diff)
{
    ...
    if (_followMovementTimer.Passed())
    {
        ...
        else if (owner->HasUnitState(UNIT_STATE_FOLLOW_MOVE))
        {
            ...
            const auto* transport = owner->GetTransport();
            if (!_faceTarget)
            {
                if (!IsFarFromFollowDistance(owner, target, _distance))
                    owner->StopMoving();
    ...
    while (uint32 eventId = _events.ExecuteEvent())
    {
        switch (eventId)
        {
            ...
            {
                // alistar: don't align for creatures with formations
                // we can just check for !Isplayer if we don't want creature pets to align with them
                if (!target->IsPlayer() && _joinFormation)
                    break;
...
void FollowMovementGenerator::LaunchMovement(Unit* owner)
{
    ...
    if (target->IsPlayer())
    {
        ...
        // Alistar: if our pet enters a transport teleport it and ignore pathfinding on transport.
        // TODO: fix pathfinding with transports
        if (owner->HasUnitState(UNIT_STATE_FOLLOW) && owner->GetTransport() != target->GetTransport())
        {
            if (auto oldTransport = owner->GetTransport())
            {
                owner->ClearUnitState(UNIT_STATE_IGNORE_PATHFINDING);
                oldTransport->RemovePassenger(owner);
            }

            owner->NearTeleportTo(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), target->GetOrientation());

            if (auto newTransport = target->GetTransport())
            {
                owner->AddUnitState(UNIT_STATE_IGNORE_PATHFINDING);
                newTransport->AddPassenger(owner);
                owner->m_movementInfo.transport.pos.Relocate(target->m_movementInfo.transport.pos);
            }
        }
    }
    ...
    {
        // alistar: add 0.5% of player speed to pet so it aligns better with you
        float targetSpeed = target->GetSpeed(SelectSpeedType(target->m_movementInfo.GetMovementFlags()));
        velocity = std::max(velocity, targetSpeed * 1.005f); // match or exceed target's pace
    ...
    if (target->IsPlayer())
    {
        // alistar: if we're falling for more than 1.1 seconds don't generate path.
        // this will make our pet ignore pathfinding and get to us as fast as possible.
        const uint32 fall_time = target->m_movementInfo.GetFallTime();
        const bool generate_path = fall_time < 1100;
        init.MoveTo(dest.GetPositionX(), dest.GetPositionY(), dest.GetPositionZ(), generate_path);
    }
    else
