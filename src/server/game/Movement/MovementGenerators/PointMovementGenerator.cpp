...
void PointMovementGenerator::Finalize(Unit* owner, bool active, bool movementInform)
{
    ...
    // alistar: start an attack after finishing the charge
    if (active && owner->ToPlayer() && i_faceTarget)
    {
        if (Unit* target = ObjectAccessor::GetUnit(*owner, i_faceTarget->GetGUID()))
        {
            // reset the previous timer
            owner->setAttackTimer(BASE_ATTACK, 0);
            owner->setAttackTimer(OFF_ATTACK, 0);
...
void AssistanceMovementGenerator::Finalize(Unit* owner, bool active, bool movementInform)
{
    ...
    if (owner->IsCreature())
    {
        ...
        owner->SetIsFleeingForAssist(false);
    }
}
