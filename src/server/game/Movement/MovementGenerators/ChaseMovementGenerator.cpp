...
bool ChaseMovementGenerator::Update(Unit* owner, uint32 diff)
{
    ...
    // ------------------------ This segment will handle the clearing of the chase state and the movement inform of the previous update cycle
    ...
    if (_positionCheckTimer.Passed())
    {
        ...
        // alistar: if we're a players pet only attack from behind when our target isn't targeting us
        if (owner->IsPet())
        {
            if (owner->GetOwner())
            {
                if (owner->GetOwner()->ToPlayer())
                {
                    if (_angle.has_value())
                    {
                        if (target->GetTarget() == owner->GetGUID())
                            _angle = ChaseAngle(0.0f, float(M_PI_4));
                        else
                            _angle = ChaseAngle(float(M_PI), float(M_PI_4));
                    }
                }
            }
            else
        ...
        if (target->isMoving())
        {
            ...
            target->MovePositionToFirstCollision(destination, SelectMovementSpeed(owner) * 0.2f, angle); // alistar: 0.2f seems good
        }
        else
        {
        ...
        // Alistar: this is called when we arrive at our target
        if (result == CHASE_POSITION_CHECK_RESULT_VALID)
        {
            if (HasFlag(MOVEMENTGENERATOR_FLAG_INFORM_ENABLED))
            {
                ...
                // alistar: creature fanning
                if (owner->GetExactDist(target) <= GetHitboxSum(owner, target) + float(CONTACT_DISTANCE))
                {
                    owner->chaseOriginalAngle = target->GetAbsoluteAngle(owner);
