...
void Quest::LoadQuestTemplateAddon(Field* fields)
{
    ...
    // alistar: 3.4.3 client doesn't play well with these flags
...
{
    return XPValue(player ? player->GetLevel() : 0, player->GetQuestLevel(this), _rewardXPDifficulty, _rewardXPMultiplier);
}

/*static*/ uint32 Quest::XPValue(uint8 playerLevel, int32 questLevel, uint32 xpDifficulty, float xpMultiplier /*= 1.0f*/)
{
    int32 quest_level = (questLevel == -1 ? playerLevel : questLevel);
...
uint32 Quest::GetRewMoneyMaxLevel(Player const* player /*= nullptr*/) const
{
    ...
    return XPValue(playerLevel, _level, _rewardXPDifficulty, _rewardXPMultiplier) * 6;
}
...
bool Quest::IsRaidQuest(Difficulty difficulty) const
{
    switch (_questInfoID)
    {
        ...
        {
            return difficulty == DIFFICULTY_10_N      ||
                   difficulty == DIFFICULTY_10_HC     ||
                   difficulty == DIFFICULTY_RAID_10_N ||
                   difficulty == DIFFICULTY_RAID_10_HC;
        }
        case QUEST_INFO_RAID_25:
        {
            return difficulty == DIFFICULTY_25_N      || 
                   difficulty == DIFFICULTY_25_HC     ||
                   difficulty == DIFFICULTY_RAID_25_N ||
                   difficulty == DIFFICULTY_RAID_25_HC;
        }
        default:
            break;
    }
...
WorldPacket Quest::BuildQueryData(LocaleConstant loc, Player* player) const
{
    ...
    const uint32 reward_money_difficulty = GetRewMoneyDifficulty();
    const float money_multiplier = GetMoneyMultiplier();
    ...
    if (!HasFlag(QUEST_FLAGS_HIDE_REWARD))
    {
        ...
        if (QuestMoneyRewardEntry const* money = sQuestMoneyRewardStore.LookupEntry(_level))
            rewardMoney = money->Difficulty[reward_money_difficulty] * money_multiplier;

        response.Info.RewardMoney = (player ? player->GetQuestMoneyReward(this) : rewardMoney * sWorld->getRate(RATE_MONEY_QUEST));
    }

    response.Info.RewardMoneyDifficulty = reward_money_difficulty;
    response.Info.RewardMoneyMultiplier = money_multiplier;
