...
AuctionsBucketKey AuctionsBucketKey::ForItem(Item const* item)
{
    ...
    if (itemTemplate->GetMaxStackSize() == 1)
    {
        ...
        {
            item->GetEntry(),
            uint16(Item::GetItemLevel(itemTemplate, *item->GetBonus(), 0, item->GetRequiredLevel(), 0, 0, 0)),
    ...
    std::span<uint8 const> knownPetBits, uint8 maxKnownPetLevel, uint32 offset, std::span<WorldPackets::AuctionHouse::AuctionSortDef const> sorts) const
{
    ...
    {
        if (resultCount >= uint32(AuctionHouseResultLimits::Browse))
            break;

        for (AuctionPosting* auction : resultBucket->Auctions)
        {
            listBucketsResult.Items.emplace_back();

            WorldPackets::AuctionHouse::AuctionItem& auctionItem = listBucketsResult.Items.back();
            auction->BuildAuctionItem(&auctionItem, false, false, auction->OwnerAccount != player->GetSession()->GetAccountGUID(),
                                      auction->Bidder.IsEmpty());
    ...
    if (filters.HasFlag(AuctionHouseFilterMask::UsableOnly))
        listBucketsResult.OnlyUsable = 1;

    listBucketsResult.TotalCount = totalAuctions;
}
...
void AuctionHouseObject::SendAuctionWon(AuctionPosting const* auction, Player* bidder, 
                                        CharacterDatabaseTransaction trans) const
{
    ...
    if ((bidder || bidderAccId) && !sAuctionBotConfig->IsBotChar(auction->Bidder))
    {
        ...
        if (bidder)
        {
            ...
            // alistar: wave on winning
            bidder->HandleEmoteCommand(EMOTE_ONESHOT_WAVE);
        }
