...
void Battleground::EndBattleground(Team winner)
{
    ...
    pvpMatchStatistics.Data.Winner = _winnerTeamId;
...
bool Battleground::AddSpiritGuide(uint32 type, float x, float y, float z, float o, TeamId teamId /*= TEAM_NEUTRAL*/)
{
    ...
    if (Creature* creature = AddCreature(entry, type, x, y, z, o, teamId))
    {
        ...
        //creature->SetChannelObjectGuid());
        // aura
        /// @todo Fix display here
        // creature->SetVisibleAura(0, SPELL_SPIRIT_HEAL_CHANNEL_AOE);
        // casting visual effect
        creature->SetChannelSpellId(SPELL_SPIRIT_HEAL_CHANNEL_AOE);
        // correct cast speed
        creature->SetModCastingSpeed(1.0f);

        // set spellvisual
        if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(SPELL_SPIRIT_HEAL_CHANNEL_AOE, creature->GetMap()->GetDifficultyID()))
            creature->SetChannelSpellXSpellVisualId(spellInfo->GetSpellXSpellVisualId());

        //creature->CastSpell(creature, SPELL_SPIRIT_HEAL_CHANNEL_AOE, true);
        return true;
    }
...
void Battleground::HandleTriggerBuff(ObjectGuid go_guid)
{
    ...
    if (index < 0)
    {
        TC_LOG_ERROR("bg.battleground", "Battleground::HandleTriggerBuff: cannot find buff gameobject ({}, entry: {}, type: {}) in internal data for BG (map: {}, instance id: {})!",
                     go_guid.ToString(), obj->GetEntry(), obj->GetGoType(), m_Map->GetId(), m_InstanceID);
        return;
    }
