...
namespace
{
    constexpr uint8 WSG_GAME_LENGTH = 25; // in minutes
}
...
void BattlegroundWS::PostUpdateImpl(uint32 diff)
{
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        {
            ++_minutesElapsed;
            UpdateWorldState(BG_WS_STATE_TIMER, WSG_GAME_LENGTH - _minutesElapsed);
        }
...
void BattlegroundWS::StartingEventCloseDoors()
{
    ...
    UpdateWorldState(BG_WS_STATE_TIMER, WSG_GAME_LENGTH); // alistar: appearently we don't need a timestamp here, it accepts minutes
}
...
void BattlegroundWS::EventPlayerCapturedFlag(Player* player)
{
    ...
    if (team == ALLIANCE)
    {
        ...
        // hack..
        if ((GameTime::GetGameTime() - m_QuickCap[player->GetGUID()]) < 75)
        {
            const uint32 achievId = (player->GetTeam() == ALLIANCE ? 202 : 1502);
            if (!player->HasAchieved(achievId))
            {
                if (AchievementEntry const* achievementEntry = sAchievementStore.LookupEntry(achievId))
...
void BattlegroundWS::HandleFlagRoomCapturePoint(int32 team)
{
    ...
    if (flagCarrier && flagCarrier->IsInAreaTrigger(sAreaTriggerStore.LookupEntry(areaTrigger)))
...
void BattlegroundWS::EventPlayerClickedOnFlag(Player* player, GameObject* target_obj)
{
        ...
        && BgObjects[BG_WS_OBJECT_A_FLAG] == target_obj->GetGUID())
    {
        ...
        m_QuickCap[player->GetGUID()] = GameTime::GetGameTime();
...
void BattlegroundWS::HandleAreaTrigger(Player* player, uint32 trigger, bool entered)
{
    ...
    switch (trigger)
    {
        ...
        case 3646:                                          // Alliance Flag spawn
            if (_flagState[TEAM_HORDE] == 2 && _flagState[TEAM_ALLIANCE] == 1)
                if (GetFlagPickerGUID(TEAM_HORDE) == player->GetGUID())
                    EventPlayerCapturedFlag(player);
            break;
        case 3647:                                          // Horde Flag spawn
            if (_flagState[TEAM_ALLIANCE] == 2 && _flagState[TEAM_HORDE] == 1)
                if (GetFlagPickerGUID(TEAM_ALLIANCE) == player->GetGUID())
                    EventPlayerCapturedFlag(player);
            break;
        case 3649:                                          // unk1
