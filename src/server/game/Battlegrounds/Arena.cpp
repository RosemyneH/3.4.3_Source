    ...
    if (isRated())
    {
        ...
        if (winnerArenaTeam && loserArenaTeam && winnerArenaTeam != loserArenaTeam)
        {
            ...
            if (winner != 0)
            {
                ...
                TC_LOG_DEBUG("bg.arena", "Arena match Type: {} for Team1Id: {} - Team2Id: {} ended. WinnerTeam: {}. Winner rating: +{}, Loser rating: {}",
                    GetArenaType(), GetArenaTeamIdByIndex(TEAM_ALLIANCE), GetArenaTeamIdByIndex(TEAM_HORDE), winnerTeam, winnerChange, loserChange);
