...
enum ItemSubclassArmor
{
    ...
    ITEM_SUBCLASS_ARMOR_RELIC                   = 11, // Alistar: TODO remove this it doesn't exist in 3.4.3
};
...
struct TC_GAME_API ItemTemplate
{
    ...
    int32 GetTotalAPBonus() const { return TotalAP; }

    // Stats
    uint16 GetArmor() const { return BasicData->Resistances[0]; }
    ...
    uint16 GetDamageType(uint8 index) const { return BasicData->DamageType[index]; }
    ...
    bool IsRangedWeapon() const
    {
        return IsWeapon() &&
               (GetSubClass() == ITEM_SUBCLASS_WEAPON_BOW ||
                ...
                GetSubClass() == ITEM_SUBCLASS_WEAPON_WAND ||
                GetSubClass() == ITEM_SUBCLASS_WEAPON_THROWN);
    }
