    ...
    switch (uState)
    {
        ...
        case ITEM_CHANGED:
        {
            ...
            for (uint8 i = 0; i < MAX_ENCHANTMENT_SLOT; ++i)
            {
                ...
                {
                    // alistar: we need to save guild charter id and signatures
                    Petition* petition = sPetitionMgr->GetPetition(GetGUID());
                    if (i == 0 && petition)
                    {
                        std::string petitionData = std::to_string(GetGUID().GetCounter());
                        petitionData += " 0 0 "; // alistar: don't save petition amount it doesn't go well client side

                        ssEnchants << petitionData.c_str();
                    }
                    else
                        ssEnchants << "0 0 0 ";
                }
...
uint32 Item::GetItemLevel(ItemTemplate const* itemTemplate, BonusData const& bonusData, uint32 level, uint32 fixedLevel,
    uint32 minItemLevel, uint32 minItemLevelCutoff, uint32 maxItemLevel)
{
    if (!itemTemplate)
...
uint32 Item::GetVisibleEnchantmentId(Player const* owner) const
{
    ...
    // alistar: allow getting temp enchants ids
    if (!enchantmentId)
        enchantmentId = GetEnchantmentId(TEMP_ENCHANTMENT_SLOT);
