...
bool GameObject::Create(uint32 entry, Map* map, Position const& pos, QuaternionData const& rotation, uint32 animProgress, GOState goState, uint32 artKit, bool dynamic, ObjectGuid::LowType spawnid)
{
    ...
    if (gameObjectAddon)
    {
        ...
        // alistar: override the size
        if (gameObjectAddon->Size != 0.0f)
            size = gameObjectAddon->Size;
    }
    ...
    switch (goInfo->type)
    {
        ...
        case GAMEOBJECT_TYPE_FISHINGNODE:
            SetLevel(0); // alistar: should this be 1?
            SetGoAnimProgress(255);
            break;
        ...
        case GAMEOBJECT_TYPE_FISHINGNODE:                   //17
        {
            ...
            switch (getLootState())
            {
                case GO_READY:                              // ready for loot
                {
                    ...
                    int32 playerFishingLevel = player->GetSkillValue(SKILL_FISHING);
                    ...
                    TC_LOG_DEBUG("misc", "Fishing check (level: {} area skill level: {} chance {} roll: {}", playerFishingLevel, areaFishingLevel, chance, roll);
