...
void TempSummon::InitStats(WorldObject* summoner, Milliseconds duration)
{
    ...
    if (Unit* unitSummoner = ToUnit(summoner))
    {
        ...
        if (!m_Properties->GetFlags().HasFlag(SummonPropertiesFlags::UseCreatureLevel) && (GetCreatureType() != CREATURE_TYPE_NON_COMBAT_PET)) // alistar: don't set level for non combat pets
    ...
    : TempSummon(properties, owner, isWorldObject), m_owner(owner)
{
    ...
    //InitCharmInfo(); // alistar: this shouldn't be here it messes up minion spells
}
