...
namespace Trainer
{
    ...
    Trainer::Trainer(uint32 id, Type type, uint32 requirement, std::string_view greeting, std::vector<Spell> spells) 
        : _id(id)
        , _type(type)
        ...
        else
        {
            ...
            if (speciesEntry)
            {
                player->GetSession()->GetBattlePetMgr()->AddPet(speciesEntry->ID, BattlePets::BattlePetMgr::SelectPetDisplay(speciesEntry),
                    BattlePets::BattlePetMgr::RollPetBreed(speciesEntry->ID), BattlePets::BattlePetMgr::GetDefaultPetQuality(speciesEntry->ID), trainerSpell->SpellId);
