...
Creature::Creature(bool isWorldObject) 
    ...
    m_respawnDelay(300), 
    m_corpseDelay(60), 
    m_ignoreCorpseDecayRatio(false), 
    ...
    _isMissingCanSwimFlagOutOfCombat(false), 
    _gossipMenuId(0), 
    _sparringHealthPct(0), 
    m_stringIds()
{
...
void Creature::AddToWorld()
{
    ...
    if (!IsInWorld())
    {
        ...
        // alistar: if the creature is in our always active list setActive(true)
        if (sObjectMgr->GetActiveCreature(GetEntry(), false))
        {
            setActive(true);
            activated = true;
        }

        if (!activated && sObjectMgr->GetActiveCreature(GetGUID().GetCounter(), true))
            setActive(true);
    }
}
...
void Creature::Update(uint32 diff)
{
    ...
    switch (m_deathState)
    {
        ...
        case ALIVE:
        {
            ...
            // alistar: check if pet can melee attack
            PetAI* petAI = dynamic_cast<PetAI*>(AI());
            if (petAI && petAI->CanMeleeAttack())
            {
...
bool Creature::CanCreatureAttack(Unit const* victim, bool /*force*/) const
{
    ...
    if (m_respawnedTime && (GameTime::GetGameTime() - m_respawnedTime) < 5 && !victim->IsInCombatWith(this))
        return false;
...
namespace
{
    // alistar: It took me 2 days to go through all of these factions
    // basically every one of these factions will have PvP enabled
    // TODO: Maybe move to DB?
    inline constexpr std::array<uint32, 140> g_PvPFactions =
    {
        //////////////////////////////////
        // Horde
        //////////////////////////////////
        ...
        105,  // Thunder Bluff
        232,  // Escortee
        775,  // Escortee - Events mostly like brewfest, midsummer
        877,  // Darkspear Trolls
        1134, // Undercity
        1154, // Undercity
        1214, // Frostwolf Clan - Mostly AV npcs
        1495, // Horde Generic
        1554, // Frostwolf Clan
        ...
        1074, // Orgrimmar
        1174, // Orgrimmar
        1215, // Frostwolf Clan - Alterac Valley
        1335, // Frostwolf Clan - Alterac Valley
        1494, // Horde Generic
        ...
        1076, // Darnassus
        1078, // Stormwind
        1217, // Stormpike Guard - Alterac Valley
        1334, // Stormpike Guard - Alterac Valley
        1534, // Stormpike Guard - Alterac Valley
    ...
    constexpr inline std::array<uint32, 46> g_PvPFactionsExceptions =
    {
        31031, 31886, 31885, 4500,  3513,  1747,  11940,
        16864, 16831, 16582, 16580, 1263,  3508,  3512,
        11260, 19163, 19164, 20894, 20895, 21317, 22314,
        22365, 22985, 851,   18230, 18744, 18743, 16801,
        16800, 16217, 18237, 18191, 18187, 18189, 18188,
        18190, 24519, 14729, 6497,  23973, 
        
        // Bael'dun mobs
        3374, 3375, 3377, 3378, 3392, 5849
    };

    constexpr inline std::array<uint32, 15> g_PvPModelIdExceptions =
    {
        // Orphans / Children
        221,   257,   338,   252,   262,   14589, 14616,
        23783, 20545, 20542, 19091, 19315, 7935, 251, 
        13169
    };
}
...
bool Creature::LoadCreaturesAddon()
{
    ...
    // Check for all factions that are in our array
    if (!HasPvpFlag(UNIT_BYTE2_FLAG_PVP))
    {
        if (FactionTemplateEntry const* factionTemplate = GetFactionTemplateEntry())
        {
            CreatureTemplate const* cInfo = GetCreatureTemplate();
            if (std::find(std::begin(g_PvPFactionsExceptions), std::end(g_PvPFactionsExceptions), cInfo->Entry) == std::end(g_PvPFactionsExceptions))
            {
                // Check for certain modelIds
                bool found = std::any_of(std::begin(cInfo->Models), std::end(cInfo->Models),
                                         [&](const CreatureModel& element)
                {
                    return std::find(std::begin(g_PvPModelIdExceptions), std::end(g_PvPModelIdExceptions), element.CreatureDisplayID) != std::end(g_PvPModelIdExceptions);
                });

                if (!found && (std::find(std::begin(g_PvPFactions), std::end(g_PvPFactions), factionTemplate->ID) != std::end(g_PvPFactions)))
                    ReplaceAllPvpFlags(UNIT_BYTE2_FLAG_PVP);
            }

            // Last try if we still don't have a pvp flag
            if (!HasPvpFlag(UNIT_BYTE2_FLAG_PVP))
                SetPvP((factionTemplate->Flags & FACTION_TEMPLATE_FLAG_PVP) != 0);
        }
    }
...
{
    if (loot.loot_type != LOOT_SKINNING && !IsPet() && GetCreatureDifficulty()->SkinLootID && hasLootRecipient())
