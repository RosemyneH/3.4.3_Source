    ...
    if (unitTarget == this)
    {
        switch (spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_PALADIN:
                if ((spellInfo->SpellFamilyFlags[0] & 0x00000002) && spellInfo->IconFileDataId == 135906)
                {
                    // Glyph of Blessing of Might
                    if (AuraEffect* aurEff = unitTarget->GetAuraEffect(57958, EFFECT_0))
                        duration += aurEff->GetAmount() * MINUTE * IN_MILLISECONDS;
                }
                else if ((spellInfo->SpellFamilyFlags[0] & 0x00010000) && spellInfo->IconFileDataId == 135970)
                {
...
float WorldObject::MeleeSpellMissChance(Unit const* /*victim*/, WeaponAttackType /*attType*/, int32 /*skillDiff*/, SpellInfo const* /*spellInfo*/) const
{
    return 0.0f;
}
...
SpellMissInfo WorldObject::MagicSpellHitResult(Unit* victim, SpellInfo const* spellInfo) const
{
    ...
    if (spellInfo->HasAttribute(SPELL_ATTR3_NO_AVOIDANCE) ||
        spellInfo->HasAttribute(SPELL_ATTR7_NO_ATTACK_MISS))
    {
        return SPELL_MISS_NONE;
    }

    // alistar: totems are immune to taunts
    if (IsPlayer() && spellInfo->HasEffect(SPELL_EFFECT_ATTACK_ME) && victim->IsTotem())
...
bool RedirectSpellEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    ...
    else if (Unit* auraOwner = ObjectAccessor::GetUnit(_self, _auraOwnerGUID))
    {
        ...
        {
            if (aurEff == _auraEffect)
            {
                aurEff->GetBase()->DropCharge(AURA_REMOVE_BY_DEFAULT);
                return true;
            }
        }
    }
...
Unit* WorldObject::GetMagicHitRedirectTarget(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    for (AuraEffect const* aurEff : magnetAuras)
    {
        if (Unit* magnet = aurEff->GetBase()->GetCaster())
        {
            ...
            {
                Milliseconds queueTime = magnet->m_Events.CalculateTime(100ms);
                if (spellInfo->HasHitDelay())
                {
                    float dist = GetDistance(magnet->GetPositionX(), magnet->GetPositionY(), magnet->GetPositionZ());
                    if (dist < 5.0f)
                        dist = 5.0f;
                    queueTime = magnet->m_Events.CalculateTime(Milliseconds(uint64(std::floor(dist / spellInfo->Speed * 1000.0f))));
                }

                magnet->m_Events.AddEvent(new RedirectSpellEvent(*magnet, victim->GetGUID(), aurEff), queueTime);
...
{
    return GetCombatReach() ? GetCombatReach() : DEFAULT_PLAYER_BOUNDING_RADIUS * GetObjectScale();
}

void WorldObject::MovePosition(Position &pos, float dist, float angle, float maxHeightChange)
{
...
void WorldObject::PlayDistanceSound(uint32 soundId, Player const* target /*= nullptr*/) const
{
    if (target)
        ...
        PlayObjectSound(soundId, GetGUID()); // alistar: for some reason speakerbot wasn't working
...
void WorldObject::GetCombatPosition(Unit const* searcher, float& x, float& y, float& z, float absAngle, float basedist)
{
    ASSERT(searcher);
    ...
    if (auto npc = searcher->ToCreature())
    {
        if (npc->CanFly())
        {
            ...
            float ground_z = GetMap()->GetHeight(searcher->GetPhaseShift(), searcher->m_positionX, searcher->m_positionY, searcher->m_positionZ, true);
            bool nearGround = std::abs(searcher->m_positionZ - ground_z) < NEAR_GROUND_THRESHOLD;
            if (nearGround)
            {
                ground_z = GetMap()->GetHeight(GetPhaseShift(), m_positionX, m_positionY, m_positionZ, true);
            ...
            if (x || y || z)
            {
                ...
                if (nearGround)
                    z = GetMap()->GetHeight(searcher->GetPhaseShift(), x, y, m_positionZ, true);
                else
