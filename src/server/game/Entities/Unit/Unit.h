...
class TC_GAME_API Unit : public WorldObject
{
    ...
    public:
        ...
        bool IsWithinMeleeRange(Unit const* obj, bool leeway = false) const { return IsWithinMeleeRangeAt(GetPosition(), obj, leeway); }
        bool IsWithinMeleeRangeAt(Position const& pos, Unit const* obj, bool leeway = false) const;
        ...
        bool ValidatePower(Powers power, uint32& index) const;
        ...
        void CalculateMeleeDamage(Unit* victim, CalcDamageInfo* damageInfo, WeaponAttackType attackType = BASE_ATTACK, bool forceCrit = false);
        ...
        float MeleeSpellMissChance(Unit const* victim, WeaponAttackType attType, int32 skillDiff, SpellInfo const* spellInfo) const override;
        ...
        int32 SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, int32 pdamage, DamageEffectType damagetype, SpellEffectInfo const& spellEffectInfo, Optional<float> const& donePctTotal, uint32 stack = 1) const;
        ...
        int32 SpellHealingBonusDone(Unit* victim, SpellInfo const* spellProto, int32 healamount, DamageEffectType damagetype, SpellEffectInfo const& spellEffectInfo, Optional<float> const& donePctTotal, uint32 stack = 1) const;
        ...
        float SpellCritChanceDone(const Unit* victim, SpellInfo const* spellInfo, SpellSchoolMask schoolMask, WeaponAttackType attackType = BASE_ATTACK, bool isPeriodic = false) const;
        ...
        uint8 GetComboPoints(Unit const* who = nullptr) const { return (who && m_comboTarget != who) ? 0 : GetPower(POWER_COMBO_POINTS); }
        uint8 GetComboPoints(ObjectGuid const& guid) const { return (m_comboTarget && m_comboTarget->GetGUID() == guid) ? GetPower(POWER_COMBO_POINTS) : 0; }
        ...
        uint32 chaseRepositionCounter;
        float chaseOriginalAngle;
        ...
        bool changedIntoShapeshift;

        // alistar: for mobs that cancel their cast after are about to flee for assist
        bool IsFleeingForAssist() const { return  m_IsFleeingForAssist; }
        void SetIsFleeingForAssist(bool val) { m_IsFleeingForAssist = val; }
        bool HasFledForAssist() const { return  m_JustFledForAssist; }
        void SetHasFledForAssist(bool val) { m_JustFledForAssist = val; }
    ...
    private:
        ...
        void UpdateChaserCombatPosition();
    ...
    private:
        ...
        bool m_IsFleeingForAssist;
        bool m_JustFledForAssist;
};

class RedirectSpellEvent : public BasicEvent
{
public:
    RedirectSpellEvent(Unit& self, ObjectGuid auraOwnerGUID, const AuraEffect* auraEffect)
        : _self(self)
