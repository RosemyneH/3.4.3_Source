...
int32 Unit::GetCreatePowerValue(Powers power) const
{
    ...
    case POWER_HAPPINESS:
        return (GetTypeId() != TYPEID_UNIT || !ToCreature()->IsPet() || ToPet()->getPetType() != HUNTER_PET) ? 0 : powerType->MaxBasePower;
    default:
        break;
    }
...
bool Player::UpdateAllStats()
{
    ...
    {
        UpdatePowerRegen(POWER_RUNE_BLOOD);
        UpdatePowerRegen(POWER_RUNE_FROST);
        UpdatePowerRegen(POWER_RUNE_UNHOLY);
    }
    ...
    else
    {
        switch (GetClass())
        {
            ...
            case CLASS_DRUID:
            {
                ...
                if (IsInFeralForm())
                {
                    if (AuraEffect const* levelMod = GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_DRUID, 132185, EFFECT_0))
                    ...
                    if (m_baseFeralAP)
                    {
                        ...
                        {
                            if (AuraEffect const* weaponMod = GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_DRUID, 132185, EFFECT_1))
                            {
...
{
    const auto applyCritLimit = [&](BaseModGroup modGroup, CombatRating cr) -> float
    {
    ...
    switch (attType)
    {
        case OFF_ATTACK:
            SetUpdateFieldStatValue(m_values.ModifyValue(&Player::m_activePlayerData).ModifyValue(&UF::ActivePlayerData::OffhandCritPercentage),
                applyCritLimit(OFFHAND_CRIT_PERCENTAGE, CR_CRIT_MELEE));
            break;
        case RANGED_ATTACK:
...
const std::array<float, MAX_CLASSES> m_diminishing_k =
{
    0.9560f,  // Warrior
    0.9560f,  // Paladin
    0.9880f,  // Hunter
...
// helper function
float CalculateDiminishingReturns(const std::array<float, MAX_CLASSES>& capArray, uint8 playerClass, float nonDiminishValue, float diminishValue)
{
    //  1     1     k              cx
    // --- = --- + --- <=> x' = --------
    //  x'    c     x            x + ck
...
const std::array<float, MAX_CLASSES> miss_cap =
{
...
const std::array<float, MAX_CLASSES> parry_cap =
{
    47.003525f,     // Warrior
    47.003525f,     // Paladin
    145.560408f,    // Hunter
...
const std::array<float, MAX_CLASSES> dodge_cap =
{
    88.129021f,     // Warrior
    88.129021f,     // Paladin
    145.560408f,    // Hunter
...
float Player::GetSpellCritFromIntellect() const
{
    ...
    GtChanceToSpellCritBase const* critBase = sChanceToSpellCritBase.GetRow(1);
    GtChanceToSpellCrit const* critRatio = sChanceToSpellCrit.GetRow(level);

    if (critBase == nullptr || critRatio == nullptr)
        return 0.0f;

    float critBase_val = 0.0f;
    float critRatio_val = 0.0f;

    switch (pclass)
    {
    case CLASS_WARRIOR:
        critBase_val = critBase->Warrior;
        critRatio_val = critRatio->Warrior;
        break;
    case CLASS_PALADIN:
        critBase_val = critBase->Paladin;
        critRatio_val = critRatio->Paladin;
        break;
    case CLASS_HUNTER:
        critBase_val = critBase->Hunter;
        critRatio_val = critRatio->Hunter;
        break;
    case CLASS_ROGUE:
        critBase_val = critBase->Rogue;
        critRatio_val = critRatio->Rogue;
        break;
    case CLASS_PRIEST:
        critBase_val = critBase->Priest;
        critRatio_val = critRatio->Priest;
        break;
    case CLASS_DEATH_KNIGHT:
        critBase_val = critBase->DeathKnight;
        critRatio_val = critRatio->DeathKnight;
        break;
    case CLASS_SHAMAN:
        critBase_val = critBase->Shaman;
        critRatio_val = critRatio->Shaman;
        break;
    case CLASS_MAGE:
        critBase_val = critBase->Mage;
        critRatio_val = critRatio->Mage;
        break;
    case CLASS_WARLOCK:
        critBase_val = critBase->Warlock;
        critRatio_val = critRatio->Warlock;
        break;
    case CLASS_DRUID:
        critBase_val = critBase->Druid;
        critRatio_val = critRatio->Druid;
        break;
    default:
        break;
    }

    float crit = critBase_val + GetStat(STAT_INTELLECT) * critRatio_val;
    return crit * 100.0f;
}
...
static std::pair<float, Optional<Rates>> const powerRegenInfo[MAX_POWERS] =
{
    ...
    { -12.5f,   RATE_POWER_RUNIC_POWER_LOSS  }  // POWER_RUNIC_POWER,    -1.25 runic power per second
};
...
void Player::UpdatePowerRegen(Powers powerType)
{
    ...
    if (HasAuraTypeWithValue(SPELL_AURA_PREVENT_REGENERATE_POWER, powerType))
    {
        SetUpdateFieldValue(m_values.ModifyValue(&Unit::m_unitData).ModifyValue(&UF::UnitData::PowerRegenFlatModifier, powerIndex), powerType == POWER_ENERGY ? -10.f : 0.f);
        SetUpdateFieldValue(m_values.ModifyValue(&Unit::m_unitData).ModifyValue(&UF::UnitData::PowerRegenInterruptedFlatModifier, powerIndex), powerType == POWER_ENERGY ? -10.f : 0.f);
        return;
    }
    ...
    switch (powerType)
    {
        ...
        case POWER_RUNIC_POWER:
        {
            result_regen                = powerRegenInfo[AsUnderlyingType(powerType)].first;
            result_regen_interrupted    = 0.f;

            result_regen *= GetTotalAuraMultiplierByMiscValue(SPELL_AURA_MOD_POWER_REGEN_PERCENT, AsUnderlyingType(powerType));
            result_regen_interrupted += static_cast<float>(GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_POWER_REGEN, AsUnderlyingType(powerType))) / 5.f;

            if (powerType != POWER_RUNIC_POWER) // Butchery requires combat
        ...
        case POWER_RUNE_UNHOLY:
        {
            ...
            result_regen = float(1 * IN_MILLISECONDS) / float(RUNE_BASE_COOLDOWN * 10'000) - powerTypeEntry->RegenPeace;
            result_regen_interrupted = float(1 * IN_MILLISECONDS) / float(RUNE_BASE_COOLDOWN * 10'000) - powerTypeEntry->RegenCombat;
            break;
        }
        default:
            break;
    ...
    if (powerRegenInfo[AsUnderlyingType(powerType)].second.has_value())
        modifier *= sWorld->getRate(powerRegenInfo[AsUnderlyingType(powerType)].second.value()); // Config rate
...
float Player::GetPowerRegen(Powers powerType) const
{
    uint32 powerIndex = GetPowerIndex(powerType);
    if (powerIndex == MAX_POWERS)
        return 0.f;
    ...
    bool interrupted = HasAuraType(SPELL_AURA_INTERRUPT_REGEN) ||
        (powerType == POWER_MANA && IsPowerRegenInterruptedByMP5Rule()) ||
        (powerType != POWER_MANA && IsInCombat());

    float regen = interrupted ? m_unitData->PowerRegenInterruptedFlatModifier[powerIndex] : m_unitData->PowerRegenFlatModifier[powerIndex];
    if (powerType != POWER_MANA)
        regen += (powerType == POWER_ENERGY || !interrupted) ? powerRegenInfo[AsUnderlyingType(powerType)].first : 0.f;
...
void Creature::CalculateMinMaxDamage(WeaponAttackType attType, bool normalized, bool addTotalPct, float& minDamage, float& maxDamage, uint8 damageIndex /*= 0*/) const
{
    ...
    auto it = GetCreatureTemplate()->difficultyStore.find(GetMap()->GetDifficultyID());
    if (it != GetCreatureTemplate()->difficultyStore.end())
        dmgMultiplier = it->second.DamageModifier;
...
bool Guardian::UpdateStats(Stats stat)
{
    ...
    if ((IsPetGhoul() || IsRisenAlly()) && (stat == STAT_STAMINA || stat == STAT_STRENGTH))
    {
        ...
        AuraEffect const* aurEff = owner->GetAuraEffect(SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE, SPELLFAMILY_DEATHKNIGHT, 237524, 0);
        if (aurEff)
        {
...
void Guardian::UpdateMaxPower(Powers power)
{
    ...
    switch (GetEntry())
    {
    case NPC_IMP:         multiplicator = 4.95f;  break;
    case NPC_VOIDWALKER:
    case NPC_SUCCUBUS:
    case NPC_FELHUNTER:
    case NPC_FELGUARD:    multiplicator = 11.5f;  break;
    ...
    if (owner != nullptr)
    {
        if (IsHunterPet())                      //hunter pets benefit from owner's attack power
        {
            ...
            if (IsPet())
            {
                ...
                if (itr != ToPet()->m_spells.end())                                 // If pet has Wild Hunt
                {
                    SpellInfo const* sProto = sSpellMgr->AssertSpellInfo(itr->first, DIFFICULTY_NONE); // Then get the SpellProto and add the dummy effect value
        ...
        else if (GetEntry() == NPC_WATER_ELEMENTAL)
        {
...
void Guardian::UpdateDamagePhysical(WeaponAttackType attType)
{
    ...
    if (Player* playerOwner = m_owner->ToPlayer())
    {
        ...
        else if (GetEntry() == NPC_FIRE_ELEMENTAL)
        {
