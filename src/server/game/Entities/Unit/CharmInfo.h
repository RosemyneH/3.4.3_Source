...
// alistar: this is probably not as correct but it works and I cba
#define UNIT_ACTION_BUTTON_ACTION(X) (uint32(X) & 0x00FFFF)
#define UNIT_ACTION_BUTTON_TYPE(X)   ((uint32(X) & 0xFFFF0000) >> 16)
#define MAKE_UNIT_ACTION_BUTTON(A, T) (uint32(A) | (uint32(T) << 16))
#define MAKE_UNIT_ACTION_BUTTON_VEHICLE(A, T) (uint32(A) | (uint32(T) << 23)) // alistar: temp we use this for vehicle spells. If we ever fix actions we wouldn't need this

struct UnitActionBarEntry
{
    UnitActionBarEntry() : packedData(uint32(ACT_DISABLED) << 16) { }
    ...
    {
        packedData = (packedData & 0xFFFF0000) | UNIT_ACTION_BUTTON_ACTION(action);
    }
};
...
struct TC_GAME_API CharmInfo
{
    public:
        ...
        int32 GetForcedSpell() const { return _forcedSpellId; }
        void SetForcedTargetGUID(ObjectGuid guid) { _forcedTargetGUID = guid; }
        ObjectGuid GetForcedTarget() const { return _forcedTargetGUID; }
