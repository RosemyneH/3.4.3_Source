...
bool Pet::LoadPetFromDB(Player* owner, uint32 petEntry, uint32 petnumber, bool current)
{
    ...
    switch (getPetType())
    {
        case SUMMON_PET:
            ...
            owner->SetLastTemporarySummonedPetNumber(petInfo->PetNumber);
            break;
        case HUNTER_PET:
...
void Pet::SavePetToDB(PetSaveMode mode)
{
    ...
    if (mode >= PET_SAVE_AS_CURRENT)
    {
        ...
        FillPetInfo(&owner->GetPetStable()->CurrentPet.value(), owner->GetTemporaryPetReactState());
...
void Pet::_LoadSpells(PreparedQueryResult result)
{
    if (result)
    {
        do
        {
            ...
            addSpell(fields[0].GetUInt32(), ActiveStates(fields[1].GetUInt32()), PETSPELL_UNCHANGED);
        }
    ...
    for (PetSpellMap::iterator itr = m_spells.begin(), next = m_spells.begin(); itr != m_spells.end(); itr = next)
    {
        ...
        switch (itr->second.state)
        {
            ...
            case PETSPELL_CHANGED:
                ...
                stmt->setUInt32(0, m_charmInfo->GetPetNumber());
                stmt->setUInt32(1, itr->first);
                stmt->setUInt32(2, itr->second.active);
                trans->Append(stmt);
...
{
    TC_LOG_DEBUG("entities.pet", "Loading auras for pet {}", GetGUID().ToString().c_str());
    ...
    */
    if (auraResult)
    {
        do
        {
            ...
            float critChance = fields[9].GetFloat();
            bool applyResilience = fields[10].GetBool();
...
void Pet::_SaveAuras(CharacterDatabaseTransaction trans)
{
    ...
    for (AuraMap::const_iterator itr = m_ownedAuras.begin(); itr != m_ownedAuras.end(); ++itr)
    {
        ...
        stmt->setFloat(index++, aura->GetCritChance());
        stmt->setBool(index++, aura->CanApplyResilience());
        trans->Append(stmt);
...
bool Pet::addSpell(uint32 spellId, ActiveStates active /*= ACT_DECIDE*/, PetSpellState state /*= PETSPELL_NEW*/, PetSpellType type /*= PETSPELL_NORMAL*/)
{
    ...
    if (TalentSpellPos const* talentPos = sDB2Manager.GetTalentSpellPos(spellId))
    {
    ...
    uint32 talentCost = sDB2Manager.GetTalentSpellCost(spellId);
    if (talentCost)
    {
...
bool Pet::removeSpell(uint32 spell_id, bool learn_prev, bool clear_ab)
{
    ...
    uint32 talentCost = sDB2Manager.GetTalentSpellCost(spell_id);
    if (talentCost > 0)
    {
