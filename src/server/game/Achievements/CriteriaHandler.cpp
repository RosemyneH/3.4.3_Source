    ...
    for (Criteria const* criteria : criteriaList)
    {
        ...
        switch (type)
        {
            ...
            case CriteriaType::EarnPersonalArenaRating:
            {
                ...
                {
                    auto it = sArenaTeamMgr->GetPersonalArenaTeams().find(referencePlayer->GetGUID());
                    if (it != sArenaTeamMgr->GetPersonalArenaTeams().end())
                    {
                        for (uint8 arena_slot = 0; arena_slot < MAX_ARENA_SLOT; ++arena_slot)
                        {
                            if (ArenaTeam::GetTypeBySlot(arena_slot) != reqTeamType)
                                continue;

                            SetCriteriaProgress(criteria, it->second[arena_slot].PersonalRating, referencePlayer, PROGRESS_HIGHEST);
                            break;
                        }
                    }
                }
...
std::span<CriteriaType const> CriteriaMgr::GetRetroactivelyUpdateableCriteriaTypes()
{
    static constexpr CriteriaType Types[] =
    {
        ...
        //CriteriaType::LearnTaxiNode, /*NYI*/
