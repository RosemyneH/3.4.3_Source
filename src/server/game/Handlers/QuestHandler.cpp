...
void WorldSession::HandleQuestgiverHelloOpcode(WorldPackets::Quest::QuestGiverHello& packet)
{
    ...
    // alistar fixes a bug with quests gossip not closing after completing:
...
void WorldSession::HandleQuestgiverAcceptQuestOpcode(WorldPackets::Quest::QuestGiverAcceptQuest& packet)
{
    ...
    if (Quest const* quest = sObjectMgr->GetQuestTemplate(packet.QuestID))
    {
        ...
        if (_player->CanAddQuest(quest, true))
        {
            ...
            // alistar: show gossip menu if the next quest is acceptable
            if (Creature* creature = object->ToCreature())
            {
                _player->PrepareQuestMenu(creature->GetGUID());
                const QuestMenu& questMenu = _player->PlayerTalkClass->GetQuestMenu();
                if (_player->GetNextAvailableQuest(questMenu))
                {
                    _player->PrepareGossipMenu(creature, _player->PlayerTalkClass->GetGossipMenu().GetMenuId(), true);
                    _player->SendPreparedGossip(creature);
                    return;
                }
            }
...
void WorldSession::HandleQuestgiverQueryQuestOpcode(WorldPackets::Quest::QuestGiverQueryQuest& packet)
{
    ...
    if (Quest const* quest = sObjectMgr->GetQuestTemplate(packet.QuestID))
    {
        ...
        if (quest->IsTurnIn())
            ...
            _player->PlayerTalkClass->SendQuestGiverQuestDetails(quest, object->GetGUID(), false, false);
    }
}
...
void WorldSession::HandlePlayerChoiceResponse(WorldPackets::Quest::ChoiceResponse& choiceResponse)
{
    ...
    if (playerChoiceResponse->Reward)
    {
        ...
        if (playerChoiceResponse->Reward->HonorPointCount)
            _player->ModifyHonorPoints(playerChoiceResponse->Reward->HonorPointCount);
