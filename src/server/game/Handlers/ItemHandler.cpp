...
void WorldSession::HandleSetAmmoOpcode(WorldPackets::Item::SetAmmo& packet)
{
    if (!_player->IsAlive())
    {
    ...
    TC_LOG_DEBUG("network", "HandleSetAmmoOpcode received ammoId {}", packet.ItemId);

    if (packet.ItemId)
    {
        if (!_player->GetItemCount(packet.ItemId))
        {
            _player->SendEquipError(EQUIP_ERR_ITEM_NOT_FOUND, nullptr, nullptr);
            return;
        }

        _player->SetAmmo(packet.ItemId);
    }
    else
        _player->RemoveAmmo();
}
