...
{
    // alistar: interrupt melee spells (heroic strike etc..)
...
void WorldSession::HandleMirrorImageDataRequest(WorldPackets::Spells::GetMirrorImageData& getMirrorImageData)
{
    ...
    if (Player* player = creator->ToPlayer())
    {
        ...
        mirrorImageComponentedData.DisplayID = getMirrorImageData.DisplayID;
        ...
        for (EquipmentSlots slot : itemSlots)
        {
            ...
            {
                if (slot == EQUIPMENT_SLOT_HEAD && player->HasPlayerFlagEx(PLAYER_FLAGS_EX_HIDE_HELM))
                    itemDisplayId = 0;
                else if (slot == EQUIPMENT_SLOT_BACK && player->HasPlayerFlagEx(PLAYER_FLAGS_EX_HIDE_CLOAK))
                    itemDisplayId = 0;
                else
