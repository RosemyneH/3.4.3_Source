...
void WorldSession::HandleBattlemasterJoinOpcode(WorldPackets::Battleground::BattlemasterJoin& battlemasterJoin)
{
    ...
    if (battlemasterJoin.PackedBattlegroundQueueTypeID == 0)
    {
        TC_LOG_ERROR("network", "Battleground: no bgtype received. possible cheater? {}", _player->GetGUID().ToString());
        return;
    }

    BattlegroundQueueTypeId bgQueueTypeId = BattlegroundQueueTypeId::FromPacked(battlemasterJoin.PackedBattlegroundQueueTypeID);
...
void WorldSession::HandleBattlemasterJoinArena(WorldPackets::Battleground::BattlemasterJoinArena& packet)
{
    ...
    if (_player->InBattleground())
    {
        ChatHandler(this).PSendSysMessage(LANG_BG_GROUP_MEMBER_ALREADY_IN_QUEUE);
        return;
    }

    const uint8 arenaType = ArenaTeam::GetTypeBySlot(packet.TeamSlot);
    ...
    ObjectGuid errorGuid = _player->GetGUID();
    ...
    if (arenaType == 0xFF)
    {
        ...
        _player->SendDirectMessage(battlefieldStatus.Write());
        return;
    }
    ...
    // only the leader queue for an arena game
    if (grp->GetLeaderGUID() != _player->GetGUID())
        return;

    // group count must match: arenaType 2 -> groupCount must be 2 etc..
    const uint32 groupCount = grp->GetMembersCount();
    if (arenaType != groupCount)
    {
        WorldPackets::Battleground::BattlefieldStatusFailed battlefieldStatus;
        sBattlegroundMgr->BuildBattlegroundStatusFailed(&battlefieldStatus, bgQueueTypeId, _player, 0, ERR_ARENA_TEAM_PARTY_SIZE, &errorGuid);
        _player->SendDirectMessage(battlefieldStatus.Write());
        return;
    }

    // alistar: TODO each queue an arena team is being created on the heap and it's only freed upon server shutdown
    // we need to clean this up, keep it on the stack and hopefully find a better solution
    // create a new arena team each queue..
    ArenaTeam* arena = new ArenaTeam(grp, arenaType);
    sArenaTeamMgr->AddArenaTeam(arena);

    const uint32 ateamId = arena->GetId();
    ...
    uint32 arenaRating = arena->GetRating();
    uint32 matchmakerRating = arena->GetAverageMMR(grp);
    ...
    GroupJoinBattlegroundResult err = grp->CanJoinBattlegroundQueue(bgTemplate, bgQueueTypeId, arenaType, arenaType, true, packet.TeamSlot, errorGuid);
    if (!err)
    {
        /*TC_LOG_DEBUG("bg.battleground", "Battleground: arena team id {}, leader {} queued with matchmaker rating {} for type {}", 
            _player->GetArenaTeamId(packet.TeamSlot), _player->GetName(), matchmakerRating, arenatype);*/
...
void WorldSession::HandleBattlemasterJoinSkirmishArena(WorldPackets::Battleground::BattlemasterJoinSkirmishArena& packet)
{
    if (DisableMgr::IsDisabledFor(DISABLE_TYPE_BATTLEGROUND, BATTLEGROUND_AA, nullptr))
    {
        ChatHandler(this).PSendSysMessage(LANG_ARENA_DISABLED);
    ...
    const uint8 arenaType = ArenaTeam::GetTypeBySlot(packet.Bracket);
    Group* grp = nullptr;
    BattlegroundTypeId bgTypeId = bgTemplate->Id;
    ...
    BattlegroundQueueTypeId bgQueueTypeIdRandom = BattlegroundMgr::BGQueueTypeId(BATTLEGROUND_RB, BattlegroundQueueIdType::Battleground, false, 0);
    ...
    const uint32 arenaRating      = 1000;
    const uint32 matchmakerRating = 1000;
    ...
    if (!packet.JoinAsGroup)
    {
        if (_player->isUsingLfg())
        {
            ...
            WorldPackets::Battleground::BattlefieldStatusFailed battlefieldStatus;
            sBattlegroundMgr->BuildBattlegroundStatusFailed(&battlefieldStatus, bgQueueTypeId, _player, 0, ERR_LFG_CANT_USE_BATTLEGROUND, &errorGuid);
            _player->SendDirectMessage(battlefieldStatus.Write());
            return;
        }
        ...
        {
            WorldPackets::Battleground::BattlefieldStatusFailed battlefieldStatus;
            sBattlegroundMgr->BuildBattlegroundStatusFailed(&battlefieldStatus, bgQueueTypeId, _player, 0, ERR_GROUP_JOIN_BATTLEGROUND_DESERTERS, &errorGuid);
            _player->SendDirectMessage(battlefieldStatus.Write());
            return;
        }
        ...
        if (_player->GetBattlegroundQueueIndex(bgQueueTypeIdRandom) < PLAYER_MAX_BATTLEGROUND_QUEUES)
        {
            // player is already in random queue
            WorldPackets::Battleground::BattlefieldStatusFailed battlefieldStatus;
            sBattlegroundMgr->BuildBattlegroundStatusFailed(&battlefieldStatus, bgQueueTypeId, _player, 0, ERR_IN_RANDOM_BG, &errorGuid);
            _player->SendDirectMessage(battlefieldStatus.Write());
            return;
        }
        ...
        if (_player->InBattlegroundQueue(true) && bgTypeId == BATTLEGROUND_RB)
        {
            // player is already in queue, can't start random queue
            WorldPackets::Battleground::BattlefieldStatusFailed battlefieldStatus;
            sBattlegroundMgr->BuildBattlegroundStatusFailed(&battlefieldStatus, bgQueueTypeId, _player, 0, ERR_IN_NON_RANDOM_BG, &errorGuid);
            _player->SendDirectMessage(battlefieldStatus.Write());
            return;
        }
        ...
        {
            WorldPackets::Battleground::BattlefieldStatusFailed battlefieldStatus;
            sBattlegroundMgr->BuildBattlegroundStatusFailed(&battlefieldStatus, bgQueueTypeId, _player, 0, ERR_BATTLEGROUND_TOO_MANY_QUEUES, &errorGuid);
            _player->SendDirectMessage(battlefieldStatus.Write());
            return;
        }
        ...
        GroupQueueInfo* ginfo = bgQueue.AddGroup(_player, nullptr, Team(_player->GetTeam()), bracketEntry, false, arenaRating, matchmakerRating, 0);
        uint32 avgTime = bgQueue.GetAverageQueueWaitTime(ginfo, bracketEntry->GetBracketId());
        ...
        WorldPackets::Battleground::BattlefieldStatusQueued battlefieldStatus;
        sBattlegroundMgr->BuildBattlegroundStatusQueued(&battlefieldStatus, _player, queueSlot, ginfo->JoinTime, bgQueueTypeId, avgTime, packet.JoinAsGroup);
        _player->SendDirectMessage(battlefieldStatus.Write());
    }
    else
    {
        ...
        err = grp->CanJoinBattlegroundQueue(bgTemplate, bgQueueTypeId, arenaType, arenaType, false, packet.Bracket, errorGuid);

        if (!err)
        {
            ginfo = bgQueue.AddGroup(_player, grp, Team(_player->GetTeam()), bracketEntry, false, arenaRating, matchmakerRating, 0);
            avgTime = bgQueue.GetAverageQueueWaitTime(ginfo, bracketEntry->GetBracketId());
        }
        ...
        for (GroupReference* itr = grp->GetFirstMember(); itr != nullptr; itr = itr->next())
        {
            ...
            WorldPackets::Battleground::BattlefieldStatusQueued battlefieldStatus;
            sBattlegroundMgr->BuildBattlegroundStatusQueued(&battlefieldStatus, member, queueSlot, ginfo->JoinTime, bgQueueTypeId, avgTime, packet.JoinAsGroup);
            member->SendDirectMessage(battlefieldStatus.Write());
...
void WorldSession::HandleRequestRatedPvpInfo(WorldPackets::Battleground::RequestRatedPvpInfo& /*packet*/)
{
    ...
    const auto& it = sArenaTeamMgr->GetPersonalArenaTeams().find(_player->GetGUID());
    if (it != sArenaTeamMgr->GetPersonalArenaTeams().end())
    {
        for (size_t i = 0; i != _player->GetArenaTeamSize(); ++i)
        {
            ratedPvpInfo.Bracket[i].WeeklyPlayed   = it->second.at(i).WeekGames;
            ratedPvpInfo.Bracket[i].WeeklyWon      = it->second.at(i).WeekWins;
            ratedPvpInfo.Bracket[i].SeasonPlayed   = it->second.at(i).SeasonGames;
            ratedPvpInfo.Bracket[i].SeasonWon      = it->second.at(i).SeasonWins;
            ratedPvpInfo.Bracket[i].PersonalRating = it->second.at(i).PersonalRating;
        }
    }
...
// alistar: it's an empty packet that modern client sends
void WorldSession::HandleRequestConquestFormulaConstants(WorldPackets::Battleground::RequestConquestFormulaConstants& /*packet*/)
{
}
...
void WorldSession::HandleAreaSpiritHealerQueryOpcode(WorldPackets::Battleground::AreaSpiritHealerQuery& areaSpiritHealerQuery)
{
    ...
    if (Battleground* bg = _player->GetBattleground())
        sBattlegroundMgr->SendAreaSpiritHealerQueryOpcode(_player, bg, spiritHealer->GetGUID());

    if (Battlefield* bf = sBattlefieldMgr->GetBattlefieldToZoneId(_player->GetMap(), _player->GetZoneId()))
        bf->SendAreaSpiritHealerQueryOpcode(_player, spiritHealer->GetGUID());
}
...
void WorldSession::HandleAreaSpiritHealerQueueOpcode(WorldPackets::Battleground::AreaSpiritHealerQueue& areaSpiritHealerQueue)
{
    ...
    if (Battleground* bg = _player->GetBattleground())
        bg->AddPlayerToResurrectQueue(spiritHealer->GetGUID(), _player->GetGUID());

    if (Battlefield* bf = sBattlefieldMgr->GetBattlefieldToZoneId(_player->GetMap(), _player->GetZoneId()))
        bf->AddPlayerToResurrectQueue(spiritHealer->GetGUID(), _player->GetGUID());
}
