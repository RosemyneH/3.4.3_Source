    ...
    else
    {
        if (!forcedCooldown)
        {
            ...
            if (cooldown <= Duration::zero() && categoryCooldown <= Duration::zero() && (categoryId == 76 || (spellInfo->IsAutoRepeatRangedSpell() && spellInfo->Id != 75)))
                cooldown = Milliseconds(_owner->GetBaseAttackTime(RANGED_ATTACK));
...
int32 SpellHistory::GetMaxCharges(uint32 chargeCategoryId) const
{
    ...
    return chargeCategoryEntry->MaxCharges;
}
...
int32 SpellHistory::GetChargeRecoveryTime(uint32 chargeCategoryId) const
{
    ...
    float recoveryTimeF = float(chargeCategoryEntry->ChargeRecoveryTime);
