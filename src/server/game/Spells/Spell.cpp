        ...
        case TARGET_UNIT_CHANNEL_TARGET:
        {
            for (ObjectGuid const& channelTarget : m_originalCaster->m_unitData->ChannelObjects)
            {
                ...
                if (unitTarget)
                {
                    ...
                    // check for explicit target redirection, for Grounding Totem for example
                    if (!m_spellInfo->IsPositive() || (!m_caster->IsFriendlyTo(unitTarget) && m_spellInfo->HasEffect(SPELL_EFFECT_DISPEL)))
                    {
                        Unit* redirect = nullptr;
                        switch (m_spellInfo->DmgClass)
                        {
                            case SPELL_DAMAGE_CLASS_MAGIC:
                                redirect = m_caster->GetMagicHitRedirectTarget(unitTarget, m_spellInfo);
                                break;
                            case SPELL_DAMAGE_CLASS_MELEE:
                            case SPELL_DAMAGE_CLASS_RANGED:
                                redirect = ASSERT_NOTNULL(m_caster->ToUnit())->GetMeleeHitRedirectTarget(unitTarget, m_spellInfo);
                                break;
                            default:
                                redirect = nullptr;
                                break;
                        ...
                        if (redirect && (redirect != unitTarget))
                        {
                            unitTarget = redirect;
    ...
    switch (targetType.GetTarget())
    {
        ...
        case TARGET_DEST_CASTER_MOVEMENT_DIRECTION:
        {
            ...
            if (targetType.GetTarget() == TARGET_DEST_CASTER_MOVEMENT_DIRECTION)
                angle = m_caster->m_movementInfo.GetMovementDirection();
...
void Spell::prepareDataForTriggerSystem()
{
    ...
    {
        m_procAttacker |= PROC_FLAG_DEAL_HELPFUL_PERIODIC;
...
void Spell::TargetInfo::DoDamageAndTriggers(Spell* spell)
{
    ...
    if (caster)
    {
        ...
        bool const canEffectTrigger = !spell->m_spellInfo->HasAttribute(SPELL_ATTR3_SUPPRESS_CASTER_PROCS) && spell->unitTarget->CanProc() &&
...
SpellMissInfo Spell::PreprocessSpellHit(Unit* unit, bool scaleAura, TargetInfo& hitInfo)
{
    ...
    if (Aura::BuildEffectMaskForOwner(m_spellInfo, MAX_EFFECT_MASK, unit))
    {
        ...
        hitInfo.AuraDuration = Aura::CalcMaxDuration(hitInfo.AuraSpellInfo, origCaster, &m_powerCost);
    ...
    if (uint32 aura_effmask = Aura::BuildEffectMaskForOwner(m_spellInfo, 1 << spellEffectInfo.EffectIndex, unit))
    {
        ...
        if (caster)
        {
            ...
            if (!hitInfo.HitAura)
            {
                ...
                uint32 const allAuraEffectMask = Aura::BuildEffectMaskForOwner(hitInfo.AuraSpellInfo, MAX_EFFECT_MASK, unit);
                int32 const* bp = hitInfo.AuraBasePoints;
                if (hitInfo.AuraSpellInfo == m_spellInfo)
                    bp = m_spellValue->EffectBasePoints.data();

                AuraCreateInfo createInfo(m_castId, hitInfo.AuraSpellInfo, GetCastDifficulty(), allAuraEffectMask, unit);
                createInfo
...
SpellCastResult Spell::prepare(SpellCastTargets const& targets, AuraEffect const* triggeredByAura)
{
    ...
    {
        TC_LOG_ERROR("", "(ServerSide check) Unit: GUID: {} ID: {} Name: {} Attempt to cast spell: {} Id: {}", 
                     m_caster->GetGUID().ToString(), m_caster->GetEntry(), m_caster->GetName(), m_spellInfo->SpellName->Str[LOCALE_enUS], m_spellInfo->Id);
...
void Spell::handle_immediate()
{
    ...
    // Remove used for cast item if need (it can be already NULL after TakeReagents call
    if (m_spellInfo->Id != 42336) // alistar: uhh this is really weird the water bucket item gets deleted from it's 2nd spell and it causes a weird crash
        TakeCastItem();
...
{
    m_successCast = (result == SPELL_FAILED_SUCCESS);
...
void Spell::SendSpellStart()
{
    ...
    {
        const std::pair<uint32, uint32>& ammoData = GetSpellCastDataAmmo();
        castData.AmmoDisplayID = ammoData.first;
        castData.AmmoInventoryType = ammoData.second;
    }
...
void Spell::SendSpellGo()
{
    ...
    // alistar TODO: SPELL_AURA_CONVERT_RUNE was in 3.3.5a
...
std::pair<uint32, uint32> Spell::GetSpellCastDataAmmo()
{
    ...
    if (Player const* playerCaster = m_caster->ToPlayer())
    {
        ...
        if (pItem)
        {
            ...
            {
                uint32 ammoID = m_caster->ToPlayer()->m_activePlayerData->AmmoID;
                if (ammoID)
                {
                    if (ItemEntry const* itemEntry = sItemStore.LookupEntry(ammoID))
                    {
                        ammoDisplayID = sDB2Manager.GetItemDisplayId(ammoID, 0);
        ...
        switch (spellEffectInfo.Effect)
        {
            ...
            case SPELL_EFFECT_HEAL_PCT:
                points += unitCaster->SpellHealingBonusDone(target,
                    spellInfo, spellEffectInfo.CalcValue(unitCaster, nullptr),
                    DIRECT_DAMAGE, spellEffectInfo, { }, 1);
        ...
        if (withPeriodic)
        {
            switch (spellEffectInfo.ApplyAuraName)
            {
                ...
                case SPELL_AURA_OBS_MOD_HEALTH:
                    points += unitCaster->SpellHealingBonusDone(target,
                        spellInfo, spellEffectInfo.CalcValue(unitCaster, nullptr),
                        DIRECT_DAMAGE, spellEffectInfo, { }, 1) * spellInfo->GetMaxTicks();
                    break;
...
void Spell::SendChannelStart(uint32 duration)
{
    ...
    if (!m_UniqueTargetInfo.empty() || !m_UniqueGOTargetInfo.empty())
    {
        ...
        for (TargetInfo const& target : m_UniqueTargetInfo)
        {
            ...
            // alistar: we need to add channelObject regardless
...
void Spell::TakeAmmo()
{
    ...
    if (item->GetTemplate()->BasicData->InventoryType == INVTYPE_THROWN)
    {
    ...
    else if (uint32 ammo = player->m_activePlayerData->AmmoID)
...
void Spell::TakeRunePower(bool didHit)
{
    ...
    for (uint32 i = 0; i < MAX_RUNES; ++i)
    {
        ...
        {
            player->SetRuneCooldown(i, didHit ? player->GetRuneBaseCooldown(i) : RUNE_MISS_COOLDOWN, true);
...
SpellCastResult Spell::CheckCast(bool strict, int32* param1 /*= nullptr*/, int32* param2 /*= nullptr*/)
{
    ...
    if (Unit* unitCaster = m_caster->ToUnit())
    {
        ...
        if (unitCaster->GetTypeId() == TYPEID_PLAYER && unitCaster->ToPlayer()->isMoving() && (!unitCaster->IsCharmed() || !unitCaster->GetCharmerGUID().IsCreature()))
        {
            ...
            if ((!unitCaster->HasUnitMovementFlag(MOVEMENTFLAG_FALLING_FAR) || !m_spellInfo->HasEffect(SPELL_EFFECT_STUCK)) &&
                (IsAutoRepeat() || (m_spellInfo->AuraInterruptFlags & SpellAuraInterruptFlags::Standing) != SpellAuraInterruptFlags::None))
    ...
    if (Unit* unitCaster = m_caster->ToUnit())
    {
        ...
        for (SpellEffectInfo const& spellEffectInfo : m_spellInfo->GetEffects())
        {
            if (spellEffectInfo.TargetA.GetTarget() == TARGET_UNIT_PET)
            {
                ...
                else if (!m_spellInfo->HasAttribute(SPELL_ATTR2_IGNORE_LINE_OF_SIGHT) &&
                         !m_spellInfo->HasAttribute(SPELL_ATTR5_ALWAYS_AOE_LINE_OF_SIGHT) &&
                         !pet->IsWithinLOSInMap(m_caster, LINEOFSIGHT_ALL_CHECKS, VMAP::ModelIgnoreFlags::M2))
                {
                    return SPELL_FAILED_LINE_OF_SIGHT;
                }
                break;
    ...
    // Alistar: dispel check
    {
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            if (m_spellInfo->GetEffects()[i].Effect == SPELL_EFFECT_DISPEL)
            {
                if (m_spellInfo->GetEffects()[i].IsTargetingArea() || m_spellInfo->HasAttribute(SPELL_ATTR1_INITIATES_COMBAT_ENABLES_AUTO_ATTACK))
                {
                    hasDispellableAura = true;
                    break;
                }

                dispelMask |= SpellInfo::GetDispelMask(DispelType(m_spellInfo->GetEffects()[i].MiscValue));
            }
            else if (m_spellInfo->GetEffects()[i].IsEffect())
            {
        ...
        if (!hasNonDispelEffect && !hasDispellableAura && dispelMask && !IsTriggered())
        {
            ...
            {
                Unit* unitCaster = m_caster->ToUnit();
                if (unitCaster  && (unitCaster->IsInSanctuary() || target->IsInSanctuary()) && !m_caster->IsFriendlyTo(target) && !m_caster->IsValidAttackTarget(target))
    ...
    for (SpellEffectInfo const& spellEffectInfo : m_spellInfo->GetEffects())
    {
        ...
        switch (spellEffectInfo.Effect)
        {
            ...
            case SPELL_EFFECT_FEED_PET:
            {
                ...
                if (!pet->GetCurrentFoodBenefitLevel(foodItem->GetTemplate()->ExtendedData->ItemLevel))
...
SpellCastResult Spell::CheckCasterAuras(int32* param1) const
{
    ...
    */
    ...
    else if (unitCaster->HasUnitFlag2(UNIT_FLAG2_NO_ACTIONS) && m_spellInfo->PreventionType & SPELL_PREVENTION_TYPE_NO_ACTIONS)
...
SpellCastResult Spell::CheckPower() const
{
    ...
    for (SpellPowerCost const& cost : m_powerCost)
    {
        ...
        // alistar: since when we gain runic power the amount is negative, we don't want to check it
        if (!(cost.Power == POWER_RUNIC_POWER && cost.Amount < 0) &&
            int32(unitCaster->GetPower(cost.Power)) < cost.Amount) // Check power amount
        {
...
SpellCastResult Spell::CheckItems(int32* param1 /*= nullptr*/, int32* param2 /*= nullptr*/) const
{
    ...
    else
    {
        ...
        if (proto->GetClass() == ITEM_CLASS_CONSUMABLE && target)
        {
        ...
        switch (spellEffectInfo.Effect)
        {
            ...
            case SPELL_EFFECT_WEAPON_DAMAGE_NOSCHOOL:
            {
                ...
                switch (item->GetTemplate()->GetSubClass())
                {
                    ...
                    case ITEM_SUBCLASS_WEAPON_CROSSBOW:
                    {
                        uint32 const ammo = player->m_activePlayerData->AmmoID;
                        if (!ammo)
                        {
                            // Requires No Ammo
...
void Spell::PreprocessSpellLaunch(TargetInfo& targetInfo)
{
    ...
    {
        if (!critChance)
            critChance = m_originalCaster->SpellCritChanceDone(unit, m_spellInfo, m_spellSchoolMask, m_attackType);
...
void Spell::SetSpellValue(SpellValueMod mod, int32 value)
{
    ...
    {
        m_spellValue->EffectBasePoints[mod] = m_spellInfo->GetEffect(SpellEffIndex(mod)).CalcBaseValue(value);;
        return;
    }
...
bool Spell::CanExecuteTriggersOnHit(uint8 effMask, SpellInfo const* triggeredByAura /*= nullptr*/) const
{
...
void Spell::TriggerGlobalCooldown()
{
    ...
    constexpr Milliseconds MinGCD = 1000ms;
