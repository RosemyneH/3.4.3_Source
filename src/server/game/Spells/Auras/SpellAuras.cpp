...
void AuraApplication::_InitFlags(Unit* caster, uint32 effMask)
{
    ...
    // alistar make positive auras cancelable client side. Ignore no_aura_icon because some spells like battle stance need cancelable flag
...
bool Aura::CanPeriodicTickCrit(Unit const* caster) const
{
    ...
    if (GetSpellInfo()->IconFileDataId == 132302 && GetSpellInfo()->SpellFamilyName == SPELLFAMILY_ROGUE)
        return true;
...
float Aura::CalcPeriodicCritChance(Unit const* caster) const
{
    ...
    float critChance = modOwner->SpellCritChanceDone(nullptr, GetSpellInfo(), GetSpellInfo()->GetSchoolMask(), GetSpellInfo()->GetAttackType(), true);
...
void Aura::SetLoadedState(int32 maxDuration, int32 duration, int32 charges, uint8 stackAmount, uint32 recalculateMask, float critChance, bool applyResilience, int32* amount)
{
    ...
    if (apply)
    {
        switch (GetSpellInfo()->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_MAGE:
                ...
                else if (GetSpellInfo()->SpellFamilyFlags[0] & 0x00000020 && GetSpellInfo()->GetSpellVisual() == 13)
                {
                    // Glyph of Frostbolt
                ...
                else if (GetSpellInfo()->SpellFamilyFlags[0] & 0x01000000)
                {
                    ...
                    if (GetSpellInfo()->IconFileDataId == 136071 && GetSpellInfo()->GetSpellVisual() == 12978)
                    {
                ...
                switch (GetId())
                {
                    case 44544: // Fingers of Frost
                    {
                        ...
                        args.AddSpellMod(SPELLVALUE_AURA_STACK, sSpellMgr->AssertSpellInfo(74396, DIFFICULTY_NONE)->StackAmount);
                    ...
                    case 12494: // Frostbite, synchronise with Fingers of Frost
                    {
                        ...
                        if (AuraEffect* aurEff = caster->GetAuraEffect(SPELL_AURA_PROC_TRIGGER_SPELL, SPELLFAMILY_MAGE, 236227, EFFECT_0))
                            caster->CastSpell(caster, aurEff->GetSpellInfo()->GetEffect(EFFECT_0).TriggerSpell, true);
                        break;
                    }
                    default:
                        break;
            ...
            case SPELLFAMILY_PRIEST:
                ...
                if (GetSpellInfo()->SpellFamilyFlags[0] & 0x02000000)
                {
                    ...
                    if (AuraEffect const* aurEff = caster->GetDummyAuraEffect(SPELLFAMILY_PRIEST, 252996, 1))
                    {
                ...
                if (GetSpellInfo()->SpellFamilyFlags[2] & 0x2)
                {
                    ...
                    for (Unit::AuraEffectList::const_iterator itr = TalentAuras.begin(); itr != TalentAuras.end(); ++itr)
                    {
                        if ((*itr)->GetMiscValue() == 7282)
                        {
                            ...
                            if ((*itr)->GetSpellInfo()->IconFileDataId == 132095)
                                break;
                        }
                    }
                    if (aurEff)
    ...
    else
    {
        switch (GetSpellInfo()->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_WARRIOR:
                ...
                if (GetSpellInfo()->SpellFamilyFlags[1] & 0x2)
                {
                    if (removeMode != AURA_REMOVE_BY_DEFAULT)
                    {
                        ...
                        if (caster->GetDummyAuraEffect(SPELLFAMILY_WARRIOR, 132361, 1))
                        {
                            ...
                            {
                                if ((*itr) != target)
                                    (*itr)->RemoveAurasWithFamily(SPELLFAMILY_WARRIOR, flag128(0, 0x2, 0, 0), GetCasterGUID());
                            }
                        }
                    }
                }
            ...
            case SPELLFAMILY_WARLOCK:
                ...
                {
                    if (AuraEffect* aurEff = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_WARLOCK, 136183, 0))
                    {
                        uint32 spellId = 0;
                        switch (aurEff->GetId())
                        {
            ...
            case SPELLFAMILY_PRIEST:
                ...
                {
                    // Shadow Affinity
                    if (AuraEffect const* aurEff = caster->GetDummyAuraEffect(SPELLFAMILY_PRIEST, 136205, 1))
                    {
                ...
                switch (GetId())
                {
                    case 47788: // Guardian Spirit
                        ...
                        if (AuraEffect* aurEff = player->GetAuraEffect(63231, 0))
                        {
                            ...
                            spellCooldown.Caster = player->GetGUID();
                            spellCooldown.Flags = SPELL_COOLDOWN_FLAG_NONE;
                            spellCooldown.SpellCooldowns.emplace_back(GetSpellInfo()->Id, aurEff->GetAmount() * IN_MILLISECONDS);
            ...
            case SPELLFAMILY_DEATHKNIGHT:
                ...
                // Reaping
                // Death Rune Mastery
                if (GetSpellInfo()->IconFileDataId == 135714 || GetSpellInfo()->IconFileDataId == 136195 || GetSpellInfo()->IconFileDataId == 135372)
                {
...
bool Aura::CanStackWith(Aura const* existingAura) const
{
    ...
    if (HasEffectType(SPELL_AURA_SHOW_CONFIRMATION_PROMPT))
        if (existingAura->HasEffectType(SPELL_AURA_SHOW_CONFIRMATION_PROMPT))
            return false;
...
uint32 Aura::GetProcEffectMask(AuraApplication* aurApp, ProcEventInfo& eventInfo, TimePoint now) const
{
    ...
    if (Spell const* spell = eventInfo.GetProcSpell())
    {
        ...
        if (spell->IsTriggered() && !(procEntry->AttributesMask & PROC_ATTR_TRIGGERED_CAN_PROC) && !(eventInfo.GetTypeMask() & AUTO_ATTACK_PROC_FLAG_MASK))
            if (!spell->GetSpellInfo()->HasAttribute(SPELL_ATTR3_NOT_A_PROC)) // alistar: wrong attr? on 335 it's SPELL_ATTR3_CAN_PROC_FROM_PROCS
                return 0;
