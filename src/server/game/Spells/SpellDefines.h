...
struct TC_GAME_API CastSpellTargetArg
{
    ...
    CastSpellTargetArg(float x = 0, float y = 0, float z = 0, float o = 0)
    {
        Targets.emplace();

        Position dest{ x, y, z, o };
        Targets->SetDst(std::move(dest));
    }
...
struct TC_GAME_API CastSpellExtraArgs
{
    ...
    CastSpellExtraArgs(AuraEffect const* eff, ObjectGuid const& origCaster) : TriggerFlags(TRIGGERED_FULL_MASK), OriginalCaster(origCaster) { SetTriggeringAura(eff); }
