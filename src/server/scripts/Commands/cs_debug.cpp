...
public:
    ...
    static bool HandleDebugSendOpcodeCommand(ChatHandler* handler)
    {
        ...
        const auto hexStringToLittleEndian = [](const std::string & hexStr) -> uint64
        {
            size_t length = hexStr.length();
            ASSERT(length == 2 || length == 4 || length == 8 || length == 16);
            ...
            for (size_t i = 0; i < length; i += 2) 
            {
                ...
                result |= (static_cast<uint64>(byte) << (8 * (i / 2)));
            }
            return result;
        };
        ...
        while (!parsedStream.eof() && !parsedStream.fail())
        {
            ...
            if (type == "HexBegin")
            {
                ...
                while (std::getline(parsedStream, stream))
                {
                    if (stream.size() >= 6)
                    {
                        const size_t endPos = stream.size() - 6;
                        std::string isEnd = stream.substr(endPos, 6);
                        if (isEnd == "HexEnd")
                        {
                            stream.erase(endPos, 6);
                            break;
                        }
                    }
                }

                std::string bytesStream;
                for (size_t pos = 0; pos != stream.size(); ++pos)
                {
                    if (bytesStream == "myguid")
                    {
                        data << player->GetGUID();
                        bytesStream.clear();
                        continue;
                    }
                    else if (bytesStream == "targetguid")
                    {
                        Unit* target = player->GetSelectedUnit();
                        if (target == nullptr)
                        {
                            TC_LOG_ERROR("misc", "Got targetguid but have no target.");
                            return false;
                        }
                    ...
                    else if (bytesStream == "String(")
                    {
                        std::string text;
                        while (true)
                        {
                            const char c = stream[pos];
                    ...
                    if (stream[pos] == '\t')
                    {
                        if (!bytesStream.empty())
                        {
                            uint64 value = hexStringToLittleEndian(bytesStream);

                            switch (bytesStream.size())
                            {
                            case 2: // uint8
                            {
                                ...
                                uint8 result = static_cast<uint8>(value);
                                data << uint8(result);
                            ...
                            case 4: // uint16
                            {
                                if (value < std::numeric_limits<uint16>::min() || value > std::numeric_limits<uint16>::max())
                                    throw std::out_of_range("Value out of int range");

                                uint16 result = static_cast<uint16>(value);
                                data << uint16(result);
                            ...
                            case 8: // uint32
                            {
                                if (value < std::numeric_limits<uint32>::min() || value > std::numeric_limits<uint32>::max())
                                    throw std::out_of_range("Value out of int range");
                            ...
                            case 16: // uint64
                            {
                                if (value < std::numeric_limits<uint64>::min() || value > std::numeric_limits<uint64>::max())
                                    throw std::out_of_range("Value out of int range");

                                uint64 result = static_cast<uint64>(value);
                                data << uint64(result);

                                break;
                            }
                            default:
                                TC_LOG_ERROR("", "Got unknown value {}", bytesStream.size());
                                break;
                            }
            ...
            else if (type == "targetguid")
            {
                Unit* target = player->GetSelectedUnit();
                if (target == nullptr)
                {
                    TC_LOG_ERROR("misc", "If you're using targetguid you need to target something.");
                    return false;
                }
