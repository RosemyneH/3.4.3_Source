...
class spell_hun_cobra_strikes : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& /*eventInfo*/)
    {
        ...
        SpellInfo const* triggeredSpellInfo = sSpellMgr->AssertSpellInfo(aurEff->GetSpellEffectInfo().TriggerSpell, DIFFICULTY_NONE);
...
class spell_hun_glyph_of_arcane_shot : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        Optional<SpellPowerCost> cost = procSpell->CalcPowerCost(POWER_MANA, GetCaster(), SpellSchoolMask(procSpell->SchoolMask), nullptr);
        if (!cost)
            return;
...
class spell_hun_invigoration : public SpellScript
{
    ...
    {
        if (Unit* unitTarget = GetHitUnit())
            if (AuraEffect* aurEff = unitTarget->GetDummyAuraEffect(SPELLFAMILY_HUNTER, 236184, 0))
...
class spell_hun_piercing_shots : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        {
            SpellInfo const* piercingShots = sSpellMgr->AssertSpellInfo(SPELL_HUNTER_PIERCING_SHOTS, DIFFICULTY_NONE);
...
class spell_hun_tame_beast : public SpellScript
{
    ...
    SpellCastResult CheckCast()
    {
        ...
        if (Creature* target = GetExplTargetUnit()->ToCreature())
        {
            ...
            CreatureTemplate const* creatureInfo = sObjectMgr->GetCreatureTemplate(target->GetEntry());
            if (!creatureInfo->IsTameable(caster->CanTameExoticPets(), creatureInfo->GetDifficulty(DIFFICULTY_NONE)))
                return SPELL_FAILED_BAD_TARGETS;
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        if (spellInfo->SpellFamilyFlags[2] & 0x200)
        {
            if (AuraEffect const* explosiveShot = eventInfo.GetProcTarget()->GetAuraEffect(SPELL_AURA_PERIODIC_DUMMY, SPELLFAMILY_HUNTER, 0x00000000, 0x80000000, 0x00000000, caster->GetGUID()))
            {
                ...
                amount = CalculatePct(static_cast<int32>(CalculatePct(caster->GetCreateMana(), GetSpellInfo()->PowerCosts[POWER_MANA]->PowerCostPct)), aurEff->GetAmount());
        ...
        {
            Optional<SpellPowerCost> cost = spellInfo->CalcPowerCost(POWER_MANA, GetCaster(), SpellSchoolMask(spellInfo->SchoolMask), nullptr);
            if (!cost)
                return;

            amount = CalculatePct(static_cast<int32>(cost->Amount), aurEff->GetAmount());
        }
