        ...
        class spell_warl_create_healthstone_SpellScript : public SpellScript
        {
            ...
            void HandleScriptEffect(SpellEffIndex /*effIndex*/)
            {
                if (Unit* unitTarget = GetHitUnit())
                {
                    ...
                    if (AuraEffect const* aurEff = unitTarget->GetDummyAuraEffect(SPELLFAMILY_WARLOCK, 135230, 0))
                    {
                        switch (aurEff->GetId())
                        {
...
class spell_warl_demonic_circle_summon : public AuraScript
{
    ...
    void HandleDummyTick(AuraEffect const* /*aurEff*/)
    {
        if (GameObject* circle = GetTarget()->GetGameObject(GetId()))
        {
            ...
            SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_WARLOCK_DEMONIC_CIRCLE_TELEPORT, DIFFICULTY_NONE);
        ...
        if (Creature* targetCreature = GetHitCreature())
        {
            if (targetCreature->IsPet())
            {
                ...
                switch (ci->family)
                {
                    ...
                    {
                        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_WARLOCK_DEMONIC_EMPOWERMENT_VOIDWALKER, DIFFICULTY_NONE);
...
class spell_warl_life_tap : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex effIndex)
    {
        ...
        float fmana = (float)base + caster->ToPlayer()->m_activePlayerData->ModDamageDonePos[SPELL_SCHOOL_SHADOW] * 0.5f * penalty;
...
class spell_warl_devour_magic : public SpellScript
{
    bool Validate(SpellInfo const* spellInfo) override
    {
        return ValidateSpellInfo({ SPELL_WARLOCK_GLYPH_OF_FELHUNTER, SPELL_WARLOCK_DEVOUR_MAGIC_HEAL })
    ...
    void OnSuccessfulDispel(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* owner = caster->GetOwner())
            if (owner->GetAura(SPELL_WARLOCK_GLYPH_OF_FELHUNTER))
