...
class spell_rog_quick_recovery : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        Optional<SpellPowerCost> cost = spellInfo->CalcPowerCost(POWER_ENERGY, caster, SpellSchoolMask(spellInfo->SchoolMask), nullptr);
        if (!cost)
            return;
...
class spell_rog_vanish : public AuraScript
{
    ...
    void Register() override
    {
        AfterEffectApply += AuraEffectApplyFn(spell_rog_vanish::ApplyStealth, EFFECT_0, SPELL_AURA_MOD_STEALTH, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
    }
};
