...
enum PriestSpells
{
    ...
    SPELL_PRIEST_PRAYER_OF_MENDING_JUMP             = 351575,
};
...
enum MiscSpellIcons
{
    SPELL_ICON_ID_STRENGTH_OF_WRYNN                 = 132486,
    SPELL_ICON_ID_HELLSCREAM_WARSONG                = 135818
};

enum PriestSpellVisuals
{
    SPELL_VISUAL_PRIEST_PRAYER_OF_MENDING = 107240
};
...
class spell_pri_blessed_recovery : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* triggerInfo = sSpellMgr->AssertSpellInfo(triggerSpell, DIFFICULTY_NONE);
...
class spell_pri_glyph_of_dispel_magic : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo = eventInfo.GetSpellInfo();
        if (!spellInfo || spellInfo->IconFileDataId != 135894)
            return;
...
class spell_pri_glyph_of_prayer_of_healing : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* triggeredSpellInfo = sSpellMgr->AssertSpellInfo(SPELL_PRIEST_GLYPH_OF_PRAYER_OF_HEALING_HEAL, DIFFICULTY_NONE);
...
class spell_pri_improved_spirit_tap : public AuraScript
{
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        {
            if (spellInfo->IsRankOf(sSpellMgr->AssertSpellInfo(SPELL_PRIEST_SHADOW_WORD_DEATH_R1, DIFFICULTY_NONE)) ||
                spellInfo->IsRankOf(sSpellMgr->AssertSpellInfo(SPELL_PRIEST_MIND_BLAST_R1, DIFFICULTY_NONE)))
                return true;
...
class spell_pri_mind_sear : public SpellScript
{
    ...
    {
        for (ObjectGuid const& channelTarget : GetCaster()->m_unitData->ChannelObjects)
        {
            WorldObject* target = ObjectAccessor::GetUnit(*GetCaster(), channelTarget);
            unitList.remove_if(Trinity::ObjectGUIDCheck(target->GetGUID()));
        }
    }
...
public:
    ...
    void CastPrayerOfMendingAura(Unit* caster, Unit* target, Unit* visualSender, uint8 stack) const
    {
        ...
        uint32 basePoints = caster->SpellHealingBonusDone(target, _spellInfoHeal, GetEffectValue(), HEAL, *_healEffectDummy, {});
...
class spell_pri_prayer_of_mending_dummy : public spell_pri_prayer_of_mending_SpellScriptBase
{
    ...
    void HandleEffectDummy(SpellEffIndex /*effIndex*/) const
    {
        ...
        CastPrayerOfMendingAura(caster, target, caster, 5);
    }
    ...
    void HandleHeal(AuraEffect const* aurEff, ProcEventInfo const& /*eventInfo*/)
    {
        ...
        if (Unit* caster = GetCaster())
        {
            ...
            if (stackAmount > 1)
            {
                ...
                args.SetCustomArg(uint8(newStackAmount));
...
class spell_pri_prayer_of_mending_jump : public spell_pri_prayer_of_mending_SpellScriptBase
{
    ...
    void HandleJump(SpellEffIndex /*effIndex*/) const
    {
        if (Unit* origCaster = GetOriginalCaster())
            CastPrayerOfMendingAura(origCaster, GetHitUnit(), GetCaster(), *std::any_cast<uint8>(&GetSpell()->m_customArg));
    }
...
class spell_pri_prayer_of_mending_heal : public SpellScript
{
    ...
    void HandleHeal(SpellEffIndex /*effIndex*/)
    {
        ...
        {
            AuraEffect* mendingAura = GetHitPlayer() ? GetHitPlayer()->GetAuraEffect(SPELL_PRIEST_PRAYER_OF_MENDING_AURA, EFFECT_0) : nullptr;
            int32 heal = mendingAura ? mendingAura->GetAmount() : GetEffectValue();
...
class spell_pri_t10_heal_2p_bonus : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_PRIEST_BLESSED_HEALING, DIFFICULTY_NONE);
