...
class spell_item_savory_deviate_delight : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        Unit* caster = GetCaster();
        uint32 spellId = 0;
        Gender gender = caster->GetGender();

        // If the caster has pygmy oil aura he'll always be transformed into the female version
        if (caster->HasAura(SPELL_PYGMY_OIL))
            gender = GENDER_FEMALE;

        switch (urand(1, 2))
        {
            // Flip Out - ninja
            case 1: spellId = (gender == GENDER_MALE ? SPELL_FLIP_OUT_MALE : SPELL_FLIP_OUT_FEMALE); break;
            // Yaaarrrr - pirate
            case 2: spellId = (gender == GENDER_MALE ? SPELL_YAAARRRR_MALE : SPELL_YAAARRRR_FEMALE); break;
        }
...
class spell_item_nitro_boosts : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex /* effIndex */)
    {
        ...
        bool success = true;
        if (areaEntry && areaEntry->GetMountFlags().HasFlag(AreaMountFlags::AllowFlyingMounts) && !caster->GetMap()->IsDungeon())
