...
class spell_mage_burnout : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        Optional<SpellPowerCost> cost = eventInfo.GetDamageInfo()->GetSpellInfo()->CalcPowerCost(POWER_MANA, GetCaster(), SpellSchoolMask(eventInfo.GetDamageInfo()->GetSpellInfo()->SchoolMask), nullptr);
        if (!cost)
            return;
...
class spell_mage_empowered_fire : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& /*eventInfo*/)
    {
        ...
        Unit* target = GetTarget();
        CastSpellExtraArgs args(aurEff);
        uint8 percent = sSpellMgr->AssertSpellInfo(SPELL_MAGE_EMPOWERED_FIRE_PROC, DIFFICULTY_NONE)->GetEffect(EFFECT_0).CalcValue();
...
class spell_mage_glyph_of_ice_block : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* /*aurEff*/, ProcEventInfo& eventInfo)
    {
        ...
        {
            SpellInfo const* cdSpell = sSpellMgr->GetSpellInfo(itr->first, DIFFICULTY_NONE);
...
class spell_mage_ignite : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* igniteDot = sSpellMgr->AssertSpellInfo(SPELL_MAGE_IGNITE, DIFFICULTY_NONE);
