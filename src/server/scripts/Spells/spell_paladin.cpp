    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        if (eventInfo.GetHealInfo() && eventInfo.GetHealInfo()->GetSpellInfo())
        {
            ...
            if (eventInfo.GetHealInfo()->GetSpellInfo()->SpellFamilyFlags[1] & 0x00010000)
                originalSpell = sSpellMgr->GetSpellInfo(sSpellMgr->GetSpellWithRank(SPELL_PALADIN_HOLY_SHOCK_R1, eventInfo.GetHealInfo()->GetSpellInfo()->GetRank()), DIFFICULTY_NONE);
            else
            ...
            if (originalSpell && aurEff->GetSpellInfo())
            {
                ...
                Optional<SpellPowerCost> cost = originalSpell->CalcPowerCost(POWER_MANA, GetCaster(), SpellSchoolMask(originalSpell->SchoolMask), nullptr);
                if (!cost)
                    return;

                uint32 bp = CalculatePct(cost->Amount, aurEff->GetSpellInfo()->GetEffect(EFFECT_1).CalcValue());
...
class spell_pal_infusion_of_light : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        if (SpellInfo const* spellInfo = eventInfo.GetSpellInfo())
        {
            ...
            if (spellInfo->SpellFamilyFlags[0] & 0x40000000 && spellInfo->IconFileDataId == 135907)
            {
                PreventDefaultAction();
                ...
                {
                    Unit* target = GetTarget();
                    int32 duration = sSpellMgr->AssertSpellInfo(SPELL_PALADIN_FLASH_OF_LIGHT_PROC, DIFFICULTY_NONE)->GetMaxDuration() / 1000;
...
private:
    ...
    void HandleScriptEffect(SpellEffIndex /*effIndex*/)
    {
        ...
        for (Unit::AuraEffectList::const_iterator i = auras.begin(); i != auras.end(); ++i)
        {
            ...
            {
                if (sSpellMgr->GetSpellInfo((*i)->GetAmount(), DIFFICULTY_NONE))
                {
...
class spell_pal_judgement_of_wisdom_mana : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_PALADIN_JUDGEMENT_OF_WISDOM_MANA, DIFFICULTY_NONE);
...
class spell_pal_light_s_beacon : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        uint32 healSpellId = procSpell->IsRankOf(sSpellMgr->AssertSpellInfo(SPELL_PALADIN_HOLY_LIGHT, DIFFICULTY_NONE)) ? SPELL_PALADIN_BEACON_OF_LIGHT_HEAL_1 : SPELL_PALADIN_BEACON_OF_LIGHT_HEAL_3;
...
class spell_pal_righteous_vengeance : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_PALADIN_RIGHTEOUS_VENGEANCE_DAMAGE, DIFFICULTY_NONE);
        ...
        class spell_pal_seal_of_vengeance_AuraScript : public AuraScript
        {
            ...
            */
            ...
            void HandleSeal(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
            {
                ...
                SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(DamageSpell, DIFFICULTY_NONE);
...
class spell_pal_sheath_of_light : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_PALADIN_SHEATH_OF_LIGHT_HEAL, DIFFICULTY_NONE);
        int32 amount = CalculatePct(static_cast<int32>(healInfo->GetEffectiveHeal()), aurEff->GetAmount());
...
class spell_pal_t8_2p_bonus : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_PALADIN_HOLY_MENDING, DIFFICULTY_NONE);
