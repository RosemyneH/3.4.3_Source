    ...
    private:
        ...
        void CalculateAttackPowerAmount(AuraEffect const* /* aurEff */, int32& amount, bool& /*canBeRecalculated*/)
        {
            if (Unit* pet = GetUnitOwner())
                ...
                if (Unit* owner = pet->ToPet()->GetOwner())
                {
                    int32 fire = owner->ToPlayer()->m_activePlayerData->ModDamageDonePos[SPELL_SCHOOL_FIRE] - owner->ToPlayer()->m_activePlayerData->ModDamageDoneNeg[SPELL_SCHOOL_FIRE];
                    int32 shadow = owner->ToPlayer()->m_activePlayerData->ModDamageDonePos[SPELL_SCHOOL_SHADOW] - owner->ToPlayer()->m_activePlayerData->ModDamageDoneNeg[SPELL_SCHOOL_SHADOW];
    ...
    class spell_dk_pet_scaling_01_AuraScript : public AuraScript
    {
            ...
            if (Unit* pet = GetUnitOwner())
            {
                if (pet->IsGuardian())
                {
                    if (Unit* owner = pet->GetOwner())
                    {
                        ...
                        if (AuraEffect const* aurEff = owner->GetAuraEffect(SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE, SPELLFAMILY_DEATHKNIGHT, 237524, 0))
        ...
        void CalculateStrengthAmount(AuraEffect const* /* aurEff */, int32& amount, bool& /*canBeRecalculated*/)
        {
            if (Unit* pet = GetUnitOwner())
            {
                ...
                aurEff = owner->GetAuraEffect(SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE, SPELLFAMILY_DEATHKNIGHT, 237524, 0);
                if (aurEff)
                {
