...
private:
    ...
    void Absorb(AuraEffect* /*aurEff*/, DamageInfo& dmgInfo, uint32& absorbAmount)
    {
        absorbAmount = std::min((uint64_t)CalculatePct(dmgInfo.GetDamage(), absorbPct), GetTarget()->CountPctFromMaxHealth(hpPct));
    }
...
private:
    ...
    void CalculateAmount(AuraEffect const* /*aurEff*/, int32& amount, bool& /*canBeRecalculated*/)
    {
        SpellInfo const* talentSpell = sSpellMgr->AssertSpellInfo(SPELL_DK_ANTI_MAGIC_SHELL_TALENT, DIFFICULTY_NONE);
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        if (eventInfo.GetSpellInfo() != nullptr)
            if (Player* player = eventInfo.GetActor()->ToPlayer())
                if (player->GetClass() == CLASS_DEATH_KNIGHT && player->HasFullyDepletedRune(RuneType::Blood))
                    return true;
...
class spell_dk_death_rune : public AuraScript
{
    ...
    void HandleProc(ProcEventInfo& eventInfo)
    {
        ...
        for (uint8 i = 0; i < MAX_RUNES && runesLeft; ++i)
        {
            if (GetSpellInfo()->IconFileDataId == 135372)
            {
                if (player->GetBaseRune(i) == RuneType::Blood)
                    continue;
            }
            else
            {
            ...
            --runesLeft;
            // Mark aura as used
            player->AddRuneByAuraEffect(i, RuneType::Death, aurEff, aurEff->GetAuraType(), GetSpellInfo());
        }
    }
...
class spell_dk_raise_dead : public SpellScript
{
    ...
    SpellCastResult CheckReagents()
    {
        ...
        SpellInfo const* reagentSpell = sSpellMgr->AssertSpellInfo(SPELL_DK_RAISE_DEAD_USE_REAGENT, DIFFICULTY_NONE);
        ...
        if (!player->CanNoReagentCast(reagentSpell))
        {
            for (uint32 i = 0; i < MAX_SPELL_REAGENTS; i++)
            {
                ...
                {
                    Spell::SendCastResult(player, reagentSpell, {}, ObjectGuid::Empty, SPELL_FAILED_REAGENTS);
...
private:
    ...
    void Absorb(AuraEffect* /*aurEff*/, DamageInfo& dmgInfo, uint32& absorbAmount)
    {
        ...
        if ((dmgInfo.GetDamageType() == SPELL_DIRECT_DAMAGE) && roll_chance_f(GetTarget()->ToPlayer()->m_activePlayerData->ParryPercentage))
            absorbAmount = CalculatePct(dmgInfo.GetDamage(), absorbPct);
    }
...
class spell_dk_sudden_doom : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        Unit* caster = eventInfo.GetActor();
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_DK_DEATH_COIL_R1, DIFFICULTY_NONE);
        uint32 spellId = 0;
...
class spell_dk_unholy_blight : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_DK_UNHOLY_BLIGHT_DAMAGE, DIFFICULTY_NONE);
...
class spell_dk_will_of_the_necropolis : public AuraScript
{
    ...
    bool Validate(SpellInfo const* spellInfo) override
    {
        SpellInfo const* firstRankSpellInfo = sSpellMgr->GetSpellInfo(SPELL_DK_WILL_OF_THE_NECROPOLIS_AURA_R1, DIFFICULTY_NONE);
    ...
    void Absorb(AuraEffect* /*aurEff*/, DamageInfo& dmgInfo, uint32& absorbAmount)
    {
        ...
        SpellInfo const* talentProto = sSpellMgr->AssertSpellInfo(sSpellMgr->GetSpellWithRank(SPELL_DK_WILL_OF_THE_NECROPOLIS_TALENT_R1, rank), DIFFICULTY_NONE);
...
public:
    ...
    class spell_dk_raise_ally_SpellScript : public SpellScript
    {
        ...
        void HandleSummon(SpellEffIndex effIndex)
        {
            ...
            TempSummon* summon = originalCaster->GetMap()->SummonCreature(entry, *GetHitDest(), properties, Milliseconds(duration), originalCaster, GetSpellInfo()->Id);
            if (!summon)
                return;
        ...
        void SetGhoul(SpellEffIndex /*effIndex*/)
        {
            if (Aura* aura = GetHitAura())
                if (spell_dk_raise_ally_AuraScript* script = aura->GetScript<spell_dk_raise_ally_AuraScript>())
...
{
    static char constexpr const ScriptName[] = "spell_dk_blood_tap";
public:
    spell_dk_blood_tap() : SpellScriptLoader(ScriptName) { }
    ...
    public:
        ...
        void HandleApply(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
        {
            ...
            player->AddRuneByAuraEffect(_runeIndex, RuneType::Death, aurEff, aurEff->GetAuraType(), GetSpellInfo());
        }
    ...
    public:
        ...
        void HandleEffect(SpellEffIndex effIndex)
        {
            ...
            if (player->GetCurrentRune(0) == player->GetCurrentRune(1))
                ...
                _runeIndex = player->GetCurrentRune(0) == RuneType::Blood ? 0 : 1;
        ...
        {
            if (Aura* aura = GetHitAura())
                if (spell_dk_blood_tap_AuraScript* script = aura->GetScript<spell_dk_blood_tap_AuraScript>())
...
class spell_dk_dancing_rune_weapon : public AuraScript
{
    ...
    void HandleTarget(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
    {
        ...
        for (TempSummon* temp : runeWeapons)
        {
            if (temp->IsAIEnabled())
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        {
            if (procSpell->IsRankOf(sSpellMgr->GetSpellInfo(SPELL_DK_BLOOD_STRIKE, DIFFICULTY_NONE)) ||
                procSpell->IsRankOf(sSpellMgr->GetSpellInfo(SPELL_DK_ICY_TOUCH, DIFFICULTY_NONE)) ||
                procSpell->IsRankOf(sSpellMgr->GetSpellInfo(SPELL_DK_PLAGUE_STRIKE, DIFFICULTY_NONE)) ||
                procSpell->IsRankOf(sSpellMgr->GetSpellInfo(SPELL_DK_DEATH_COIL_DAMAGE, DIFFICULTY_NONE)) ||
                procSpell->IsRankOf(sSpellMgr->GetSpellInfo(SPELL_DK_DEATH_STRIKE, DIFFICULTY_NONE)) ||
                procSpell->IsRankOf(sSpellMgr->GetSpellInfo(SPELL_DK_HEART_STRIKE, DIFFICULTY_NONE)) ||
                procSpell->IsRankOf(sSpellMgr->GetSpellInfo(SPELL_DK_OBLITERATE, DIFFICULTY_NONE)) ||
                procSpell->IsRankOf(sSpellMgr->GetSpellInfo(SPELL_DK_RUNE_STRIKE, DIFFICULTY_NONE)))
                return true;
        }
