...
class spell_dru_glyph_of_innervate : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_DRUID_GLYPH_OF_INNERVATE_REGEN, DIFFICULTY_NONE);
...
class spell_dru_lifebloom : public AuraScript
{
    ...
    void OnRemoveEffect(Unit* target, AuraEffect const* aurEff, uint32 stack)
    {
        ...
        if (Unit* caster = GetCaster())
        {
            ...
            args.AddSpellBP0(CalculatePct(caster->GetCreateMana(), GetSpellInfo()->PowerCosts[POWER_MANA]->PowerCostPct) * stack / 2);
...
class spell_dru_t3_8p_bonus : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        Unit* caster = eventInfo.GetActor();
        Optional<SpellPowerCost> cost = spellInfo->CalcPowerCost(POWER_MANA, caster, SpellSchoolMask(spellInfo->SchoolMask), nullptr);
        if (!cost)
            return;
