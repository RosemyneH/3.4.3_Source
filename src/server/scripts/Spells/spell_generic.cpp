...
{
    SPELLICON_DRUID_INFECTED_WOUNDS = 236158
};
...
class spell_gen_black_magic : public AuraScript
{
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        ProcFlagsInit typeMask = eventInfo.GetTypeMask();
        if (!spellInfo)
            return false;

        if (typeMask & PROC_FLAG_DEAL_HARMFUL_SPELL)
            return true;
        else if (typeMask & PROC_FLAG_DEAL_MELEE_ABILITY)
        {
        ...
        else if (typeMask & PROC_FLAG_DEAL_HARMFUL_ABILITY)
        {
            if (spellInfo->SpellFamilyName == SPELLFAMILY_DRUID && spellInfo->IconFileDataId == SPELLICON_DRUID_INFECTED_WOUNDS)
                return true;
        }
...
class spell_gen_blade_warding : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        Unit* caster = eventInfo.GetActionTarget();
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_GEN_BLADE_WARDING_TRIGGERED, DIFFICULTY_NONE);
            ...
            case EFFECT_1: // On damaging spells, for removing a defend layer
            {
                ...
                for (Unit::AuraApplicationMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
                {
                    if (Aura* aura = itr->second->GetBase())
                    {
                        ...
                        if (auraInfo && auraInfo->IconFileDataId == 132360 && aura->HasEffectType(SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN))
                        {
...
// alistar: TODO UpdateEncounterStateForSpellCast doesn't exist here
/*
...
class spell_gen_feign_death_all_flags : public AuraScript
{
    ...
    {
        Unit* target = GetTarget();
        target->SetDynamicFlag(UNIT_FLAG3_FAKE_DEAD);
    ...
    {
        Unit* target = GetTarget();
        target->RemoveDynamicFlag(UNIT_FLAG3_FAKE_DEAD);
...
private:
    ...
    void HandleMount(SpellEffIndex effIndex)
    {
        ...
        if (Player* target = GetHitPlayer())
        {
            ...
            SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(_mount150, DIFFICULTY_NONE);
...
enum MossCoveredHands
{
    SPELL_DROPPED_WEAPON = 6608
};

// 6866 Moss Covered Hands
// 31397 Moss Covered Hands
class spell_gen_moss_covered_hands : public AuraScript
{
    PrepareAuraScript(spell_gen_moss_covered_hands);

    bool Validate(SpellInfo const* /*spellInfo*/) override
    {
        return ValidateSpellInfo({ SPELL_DROPPED_WEAPON });
    }

    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        PreventDefaultAction();
        eventInfo.GetActor()->CastSpell(nullptr, SPELL_DROPPED_WEAPON, aurEff);
    }

    void Register() override
    {
        OnEffectProc += AuraEffectProcFn(spell_gen_moss_covered_hands::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
    }
};
...
// alistar: TODO the packet doesn't exist?
#if 0
...
{
    // Ours
    RegisterSpellScript(spell_gen_moss_covered_hands);
    ...
    //RegisterSpellScript(spell_gen_dungeon_credit); alistar: TODO
    ...
    //RegisterSpellScript(spell_gen_spirit_healer_res); alistar: TODO
