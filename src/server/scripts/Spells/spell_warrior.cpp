...
class spell_warr_bloodthirst_heal : public SpellScript
{
    ...
    {
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_WARRIOR_BLOODTHIRST_DAMAGE, DIFFICULTY_NONE);
...
class spell_warr_deep_wounds : public SpellScript
{
    ...
    void HandleDummy(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* target = GetHitUnit())
        {
            ...
            SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_WARRIOR_DEEP_WOUNDS_PERIODIC, DIFFICULTY_NONE);
...
class spell_warr_execute : public SpellScript
{
    ...
    void HandleEffect(SpellEffIndex /*effIndex*/)
    {
        ...
        if (Unit* target = GetHitUnit())
        {
            ...
            Optional<SpellPowerCost> cost = spellInfo->CalcPowerCost(POWER_RAGE, caster, SpellSchoolMask(spellInfo->SchoolMask), nullptr);
            if (!cost)
                return;

            int32 rageUsed = std::min<int32>(300 - cost->Amount, caster->GetPower(POWER_RAGE));
...
class spell_warr_extra_proc : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& /*eventInfo*/)
    {
        ...
        if (auraInfo->IsRankOf(sSpellMgr->AssertSpellInfo(SPELL_WARRIOR_BLOODSURGE_R1, DIFFICULTY_NONE)))
            target->CastSpell(nullptr, SPELL_WARRIOR_SLAM_GCD_REDUCED, aurEff);
        else if (auraInfo->IsRankOf(sSpellMgr->AssertSpellInfo(SPELL_WARRIOR_SUDDEN_DEATH_R1, DIFFICULTY_NONE)))
...
class spell_warr_victory_rush : public SpellScript
{
    ...
    void HandleBeforeCast()
    {
        if (Player* player = GetCaster()->ToPlayer())
            player->RemoveAura(SPELL_WARRIOR_VICTORIOUS);
    }

    void Register() override
    {
        BeforeCast += SpellCastFn(spell_warr_victory_rush::HandleBeforeCast);
    }
};
