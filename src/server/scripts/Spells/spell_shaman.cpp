...
class spell_sha_earthbind_totem : public AuraScript
{
    ...
    void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
    {
        ...
        if (Player* owner = GetCaster()->GetCharmerOrOwnerPlayerOrPlayerItself())
            if (AuraEffect* aur = owner->GetDummyAuraEffect(SPELLFAMILY_SHAMAN, 136024, 0))
...
class spell_sha_flame_shock : public AuraScript
{
    ...
    void HandleDispel(DispelInfo* /*dispelInfo*/)
    {
        if (Unit* caster = GetCaster())
            ...
            {
                if (SpellInfo const* firstRankSpellInfo = sSpellMgr->GetSpellInfo(SPELL_SHAMAN_LAVA_FLOWS_R1, DIFFICULTY_NONE))
...
class spell_sha_flametongue_weapon : public AuraScript
{
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        WeaponAttackType attType = Player::GetAttackBySlot(item->GetSlot(), proto->GetInventoryType());
...
class spell_sha_frozen_power : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        Unit* caster = eventInfo.GetActor();
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_SHAMAN_FREEZE, DIFFICULTY_NONE);
...
class spell_sha_glyph_of_totem_of_wrath : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* totemSpell = sSpellMgr->GetSpellInfo(totem->m_spells[0], DIFFICULTY_NONE);
        if (!totemSpell)
            return;
...
class spell_sha_item_mana_surge : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        Optional<SpellPowerCost> cost = spellInfo->CalcPowerCost(POWER_MANA, GetTarget(), SpellSchoolMask(spellInfo->SchoolMask), nullptr);
        if (!cost)
            return;

        int32 mana = cost->Amount;
...
class spell_sha_t8_elemental_4p_bonus : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_SHAMAN_ELECTRIFIED, DIFFICULTY_NONE);
...
class spell_sha_t9_elemental_4p_bonus : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_SHAMAN_LAVA_BURST_BONUS_DAMAGE, DIFFICULTY_NONE);
        int32 amount = CalculatePct(static_cast<int32>(damageInfo->GetDamage()), aurEff->GetAmount());
...
class spell_sha_t10_restoration_4p_bonus : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_SHAMAN_CHAINED_HEAL, DIFFICULTY_NONE);
...
class spell_sha_windfury_weapon : public AuraScript
{
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        WeaponAttackType attType = static_cast<WeaponAttackType>(player->GetAttackBySlot(item->GetSlot(), proto->GetInventoryType()));
        if (attType != BASE_ATTACK && attType != OFF_ATTACK)
            return false;
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        SpellInfo const* spellInfo = sSpellMgr->AssertSpellInfo(SPELL_SHAMAN_WINDFURY_WEAPON_R1, DIFFICULTY_NONE);
        while (spellInfo)
        {
