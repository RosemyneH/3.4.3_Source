...
void HotfixDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(HOTFIX_SEL_CHARACTER_SERVICE_INFO, "SELECT ID, FlowTitleLang, PopupTitleLang, PopupDescriptionLang, ServiceType, BoostType, IconFileDataID, Priority, "
                     " Flags, ProfessionLevel, BoostLevel, Expansion, PopupUITextureKitID FROM character_service_info"
                     " WHERE (`VerifiedBuild` > 0) = ?", CONNECTION_SYNCH);

    PREPARE_MAX_ID_STMT(HOTFIX_SEL_CHARACTER_SERVICE_INFO, "SELECT MAX(ID) + 1 FROM character_service_info", CONNECTION_SYNCH);
    PREPARE_LOCALE_STMT(HOTFIX_SEL_CHARACTER_SERVICE_INFO, "SELECT ID, FlowTitleLang_lang, PopupTitleLang_lang, PopupDescriptionLang_lang FROM character_service_info_locale WHERE (`VerifiedBuild` > 0) = ?"
                        " AND locale = ?", CONNECTION_SYNCH);
    ...
    // GlobalStrings.db2
    PrepareStatement(HOTFIX_SEL_GLOBAL_STRINGS, "SELECT ID, BaseTag, TagText, Flags FROM global_strings"
                     " WHERE (`VerifiedBuild` > 0) = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(HOTFIX_SEL_GROUP_FINDER_ACTIVITY, "SELECT ID, FullName, ShortName, GroupFinderCategoryID, OrderIndex, GroupFinderActivityGrpID, Unk340_Level, Flags, MinGearLevelSuggestion, PlayerConditionID, MapID, DifficultyID, AreaID, MaxPlayers, DisplayType, MinLevel, MaxLevelSuggestion, IconFileDataID FROM group_finder_activity WHERE (`VerifiedBuild` > 0) = ?", CONNECTION_SYNCH);
    ...
    // StableSlotPrices.db2
    PrepareStatement(HOTFIX_SEL_STABLE_SLOT_PRICES, "SELECT ID, Cost FROM stable_slot_prices WHERE (`VerifiedBuild` > 0) = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(HOTFIX_SEL_UI_EXPANSION_DISPLAY_INFO, "SELECT ID, ExpansionLogo, ExpansionBanner, ExpansionLevel, ReleaseType FROM ui_expansion_display_info"
                     " WHERE (`VerifiedBuild` > 0) = ?", CONNECTION_SYNCH);
    PREPARE_MAX_ID_STMT(HOTFIX_SEL_UI_EXPANSION_DISPLAY_INFO, "SELECT MAX(ID) + 1 FROM ui_expansion_display_info", CONNECTION_SYNCH);
